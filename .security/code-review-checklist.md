...
| **Frontend Core** (`src/Frontend/` — DataLayer, ScriptTag, ContainerCode, ConsentDefaults, VisitorIp, Frontend) | [-] | [x] 2026-07-10 | [x] 2026-07-14 | [-] | [x] 2026-07-14 | [x] 2026-07-10 | [x] 2026-07-10 |
...
| **ConsentMode Module** (`src/Modules/ConsentMode/` — incl. Axeptio handler) | [x] 2026-07-14 | [x] 2026-07-14 | [x] 2026-07-14 | [-] | [x] 2026-07-14 | [x] 2026-07-14 | [x] 2026-07-14 |
...
| **Module Admin Schemas** (`src/Modules/*/AdminSchema.php`) | [x] 2026-07-10 | [x] 2026-07-14 | [x] 2026-07-14 | [-] | [x] 2026-07-14 | [-] | [x] 2026-07-10 |
| **Frontend JS** (`js/frontend/`) | [-] | [x] 2026-07-14 | [x] 2026-07-14 | [-] | [x] 2026-07-14 | [x] 2026-07-13 | [-] |
...
|---|---|---|
| **Dead functions/methods** (private/public across `src/`, `compat/`, root `*.php`, `tests/`) | 2026-07-14 | Clean. Standalone `Modules/Axeptio/` removed cleanly (dropped from `Registry::BUILTIN_MODULES`, no dangling refs). New `Axeptio` handler, CF7 helpers and `AxeptioVersionControl`/`axeptioVersionOptions` all reachable. (2026-07-10: `Field::rest_type()` was dead → fixed; `Plugin::registry()`/`options()` + `Registry::get()` kept as intentional public API.) |
| **Dead hooks** (`add_action`/`add_filter` with no callback; `do_action`/`apply_filters` constants with no listener) | 2026-07-14 | Clean. New `FILTER_DEFAULT_ENABLED` (ConsentDefaults) has the Axeptio listener; new `GTM4WP_WPFILTER_AXEPTIO_CONSENT_MODE_DEFAULT` is a documented public extension point applied in `Axeptio::consent_mode_default()`; `amp_analytics_entries` is amp-wp's own filter. All prior custom `do_action`/`apply_filters` constants remain documented extension points with live internal listeners. |
| **Dead option constants** (`GTM4WP_OPTION_*`/`GTM4WP_*` in `compat/constants.php`, never read) | 2026-07-14 | Clean. New CF7 (`WPCF7_INPUTS`/`WPCF7_GA4EVENTS`) and Axeptio (`INTEGRATE_AXEPTIO`/`_PROJECTID`/`_COOKIES_VERSION`/`_CONSENTMODE`) constants are all read; `BLACKLIST_SANDBOXED` now also referenced in `Migration::REMOVED_OPTION_KEYS` (dropped feature, kept for BC + migration cleanup). All active option constants read. |
| **Dead JS** (`js/**/*.js` never enqueued / no `build/` entry) | 2026-07-13 | Clean. `webpack.config.js` uses a non-recursive `readdirSync(js/frontend)`, so `lib/native-video-params.js` (shared import) and `js/frontend/test/*` are NOT orphan bundles. All 11 media trackers enqueued by their gated block in `MediaEventsModule`. (2026-07-10: `gtm4wp-html5media.js` dead-then-wired, finding #9.) |
| **Duplication / drift** (a helper coexisting with inline copies; a module escaping the dataLayer differently from siblings) | 2026-07-14 | Drift found **and fixed**: `header_top` printed inline JS via raw `wp_kses()` (no ampersand-restore) while `header_begin`/`print_script_block()` restored it (finding #29) — now both head paths route through `print_script_block()`, so the sanitize + ampersand-restore contract is uniform. Axeptio (consent) + CF7 (dataLayer-only) sinks match their sibling patterns. (2026-07-13: media-tracker milestone/ready boilerplate; 2026-07-10: eligibility/JSON-flag/`esc_js` drift.) | + `DOMContentLoaded`-ready boilerplate ~11× (finding #24, Low — payloads are provider-specific; `native-video-params.js` already extracts the shared `gtm.video*` mapping). Also: 3 older trackers miss the `if(!duration)return` guard the 6 new ones have (finding #20). (2026-07-10: eligibility/JSON-flag/`esc_js` drift, findings #8/#11/#12/#13.) |
| **Over-abstraction** (single-caller interfaces, forward-only wrappers, unread options) | 2026-07-14 | None material. `Axeptio` as a plain ConsentMode handler (not a full module) matches the documented WooCommerce-helper pattern; `TYPE_AXEPTIO_VERSION` + `AxeptioVersionControl` justified (project-specific client-fetched dropdown). Minor: `ConsentModeModule` instantiates `new Axeptio()` even when disabled (gating is inside `register_hooks()`) — negligible. (2026-07-13: `native-video-params.js` justified; 2026-07-10: `Field::rest_type()` dead.) |
