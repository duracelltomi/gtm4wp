...
- **RI-5** — every user-facing string uses `__()`/`esc_html__()` with text domain `duracelltomi-google-tag-manager` — the schema-property `description`s of an ability included (core's own abilities and WooCommerce's wrap every one; a translated site otherwise gets a mixed-language listing; `phpcs` cannot see a *missing* call, #256).
...
- **RI-15** — an option's label and description are its consent contract: if the code emits more than the description names — or names neither the data-layer variable it writes nor the *form* the values take — that is a finding even when the data is escaped perfectly. The same holds for a status/Site Health row that restates an option: its label carries the unit the CODE applies, read from the comparison, never from the option's name (#261: "(days)" on a minutes option).
...
- **RI-31** — our own filter's argument types are fixed by the first call site; a second `apply_filters( OURS, … )` passing a different type per platform (object → string) breaks callbacks written against the first, on the path the second site reaches (#243). Tabulate types per call site and platform; say them in the docblock.
- **RI-32** — the second writer to a sanitized store (a one-field `update` beside the `record()` that cleans) is the one that skips the cleaning helper (#244). List every writer of each option row/meta/log; each routes text through the same helper.
...
- **PA-21** — a plugin-wide collector (the ability `Registrar`, the status/settings/site-health abilities, Site Health Info, the settings REST controller) keeps NO per-module map: a module's fact — its ability provider, master switch, host plugin, status or Site Health rows — comes through an opt-in interface on its `AdminSchema` walked with `instanceof` (`AbilitiesInterface`, `StatusInfoInterface`, `SiteHealthInfoInterface`, `SiteHealthTestsInterface`), or from a class inside the module; a feature-scoped ability lives in `src/Modules/<Name>/Abilities.php`. The corollary: a collector that turns a module id into an EXTERNAL identifier (an HTML id, a selector, a test id core reads back) owns the character-set rule and a collision rule, because the id is third-party input the registry never validates (#266).
...
- **PA-20** — a feature that stores a new KIND of asset (a credential, a PII queue, a file) silently invalidates the threat model's scope claims: those in/out-of-scope lines are premises with no expiry alarm, so re-read them in the same change — and give the new asset its rating vocabulary before the first finding against it needs one (#230: "the plugin stores no credentials" was seeded 2026-07-17 and falsified by the KeyVault commit with no signal).
...
**A fix's own hunk is where the straggler hides (2026-09-23, R37, #312).** The #277 fix
omitted four identity variables on both tiers and left `visitorIP` — the same class, in the
same method, sitting in the fix's own diff as unmodified context — emitting `''`. When a fix
changes N siblings, grep the method for every assignment of the same class before
committing; a sibling visible as context in the fix's diff is the cheapest possible miss.
**And a conditional tag guarantees `isset`, not a class (R37, #315):** core's `is_search()`
returns false only when `$wp_query` is unset; a foreign object answering the tag reaches an
ungated `$wp_query->post_count` read. Gate `$wp_query` property reads with `isset()` whatever
tag sits above them.
...
other tool owns `window.dataLayer` there. **The PHP writers are the same class (2026-09-23, R36, #269):** the Consent Mode default block hardcoded `dataLayer` inside a PHP-emitted `gtag` shim while the header and loader used the configured name, and sixteen `[x]` cells never selected it because this litmus said `js/`. **Litmus:** grep `js/` **and `src/`** for the *default*
...
- **A cookie's identity is name + Domain + Path, not its name (2026-09-23, R36, #270).** A PHP `setcookie()` with `domain => COOKIE_DOMAIN` and a JS clear with no `Domain` attribute address two different cookies wherever the constant is non-empty (core sets it on subdomain multisite), so the clear is a no-op and the presence gate it was meant to reset stays open. For every cookie one end writes and the other clears, diff the *attributes* of the two calls, and give the JS fixture a parent-domain cookie at a subdomain URL — jsdom's jar is host-only by default and hides it.
- **A removed literal must not come back as a parameter default (2026-09-23, R37, #317).** The #269 fix replaced the hardcoded `dataLayer` in the consent block with a `$datalayer_name` parameter and gave it the default `'dataLayer'`, so any second caller silently gets the defect back. Make the name required at every writer; the one legitimate default lives in `DataLayer::name()`.
...
**Instance ledger addition (2026-08-29, R27, #215) — the contract can be a CSS class, and the two ends can disagree for eight years.** The WooCommerce tracker's only Products-widget selector is `.widget-product-item`; the PHP side stopped emitting that class on 2017-11-26 (`85dac34`) and WooCommerce's template never had it. Nothing errored, no test went red, and the output-buffering that injects the product-data attribute kept running on every widget render for nothing. When one end of a PHP-writes/JS-reads contract is a selector, grep the *literal* on both sides — and register it as an `.upstream` claim when the markup belongs to a third party.
...
- **The inverse spelling: a failure value that is TRUTHY (2026-09-23, R36, #280).** `form.action || ''` never falls back when a form control is *named* `action`: `HTMLFormElement` is `[LegacyOverrideBuiltIns]`, so the named control shadows the property and the guard receives the element itself. jsdom does not implement the override, so no jest test can see it; read the attribute (`getAttribute()`, resolved against `document.baseURI` in a try/catch) when the value must be the URL, and treat "matches what GTM pushes" as a claim to verify in a browser.
...
- **`function_exists()` conflates "file loaded" with "feature initialised" (2026-09-23, R36, #282).** `menu_page_url()` existing proves only that `wp-admin/includes/plugin.php` was required — which a configuration check does on REST requests — not that `admin_menu` ran and registered the page; on that path core returns `''` and the fallback written for the "absent" case never fires. Gate on the positive precondition (`did_action( 'admin_menu' )`) and make the regression test model "defined but returning `''`", not "undefined".
...
- **Litmus (re-derive, don't read):** `grep -rnoE "(apply_filters|do_action)\( '[a-z_]+'" src/ compat/ | grep -v gtm4wp`
  ...
  (`pll_get_post`/`pll_get_term`/`pll_default_language`), which are outside this ledger — a
  wrong arity there is a visible fatal on our own page, not a silent contract break.
  Previously: **re-derived 2026-08-12 (R23 fix session):
...
### RI-27: A numeric-string array key is an int by the time you read it — strict compares over map keys silently miss

**Found 2026-08-29 (R27, #211)** in the post-meta loop of `PageVariablesModule` — inside the very
range R26 had just tightened, which is the point: the review of that loop looked at every *value*
transform and never at the *key's* type.

WordPress builds the meta cache with `$cache[ $id ][ $meta_key ] = …` from the raw DB column, and
PHP converts a key like `"123"` to `int(123)` on the way in (`"0"` too; `"00"` and `" 1"` stay
strings). Everything that then compares the key strictly against strings — `in_array( …, true )`,
`===`, a `match`, a strictly-typed filter callback `fn( bool $d, string $key )` — misses or throws,
and it misses *silently* because the loose-compare sibling in the next file still works. Verified by
probe and by throwaway PHPUnit runs: an int-keyed fixture failed the allow-list test on the
unmodified code; a one-line `$key = (string) $key;` at the loop top made it pass with all 66
existing tests green.

- **Litmus:** every `foreach ( $map as $key => $value )` where `$map` came from the database, a
  superglobal, or a third-party cache — ask what the key's PHP type is *now*, not what the column
  type is. If anything downstream compares it strictly or passes it to a typed signature, cast at
  the top of the loop.
- **What the cast does NOT do:** `$out[ (string) $key ] = …` re-coerces, so the output container's
  shape is unchanged; `json_encode` of a list `[0 => …]` still emits a JSON array. Only chase that
  half if the input can actually produce a `"0"` key — for post meta it cannot: `add_metadata()` /
  `update_metadata()` / the Custom Fields box all refuse an empty-ish key (`! $meta_key`).
- **Test-side tell (UC-3 shape):** the suite's `is_protected_meta` stub already casts
  `(string) $meta_key` while no fixture ever supplied an int key — somebody knew, nothing pinned
  it. A stub more permissive than the collaborator hides exactly this class.
- **Ledger (re-derive, don't read):** `grep -rnE 'foreach \( \$[a-z_]+ as \$[a-z_]+_key => ' src/`
  — 2026-08-29: **1** site over a DB-derived map (`PageVariablesModule.php:394`); the cookie
  readers (`Helpers::read_item_list_cookie()`) already `absint()` their keys.

### RI-28: An "empty / untouched" predicate must match the state the UI actually creates

Found as #232 (R31, GDM destinations table). The save-time sanitizer dropped rows where
*every* cell was empty (`'' === implode( '', $row )`) and documented that as "the
untouched Add row state" — but `TableControl.emptyRow()` seeds each select column's
schema `default`, so the untouched state the UI actually submits was
`{type:'ga4', rest empty}` and never matched. Result: the drop path was dead for
UI-created rows, and an admin who added a row without completing it got a recurring
validation error naming a row they never touched (per-field save, so only that option
failed to persist — the blast radius was measured, not assumed; the drafted
"blocks the whole save" reach claim was refuted in adjudication).

The general shape is a two-ended contract (RI-14's cousin, in-page rather than
PHP↔runtime): the sanitizer's definition of "empty" and the UI's definition of "fresh"
must be one definition. Litmus when reviewing any table/collection sanitizer:

- Enumerate every column the UI **seeds** (defaults, prefills) and ask whether the
  emptiness/untouched test excludes them.
- The regression test for the drop path must use the exact row shape the UI submits —
  the existing drop test here omitted the seeded cell and passed forever.
- When rows can be dropped, check what the error messages number: an index that
  increments only for surviving rows names "row 1" for what the screen shows as row 2.

### RI-30: An empty collection handed to somebody else's API is a predicate, not an absence
Passing `array()` where an API takes a filter does not mean "no filter" unless that API says so; for many it means "match records whose value IS the empty collection". Action Scheduler is the worked example (#240, R33): `as_unschedule_all_actions( $hook, array(), $group )` takes its bulk `cancel_actions_by_hook()` shortcut only when the *group* is empty too; with a group it loops `as_unschedule_action()`, whose `is_array( $args )` gate turns `array()` into `AND a.args = '[]'` — a predicate no action scheduled with a payload can match. The plugin's uninstall cancelled nothing on that backend for the whole of phase 4, on 3.1.6 (the WC 5.0 floor) and trunk alike, while `wp_unschedule_hook()` did its half and the tests stayed green because their alias ignored the argument. WordPress core has the same shape in the other direction (`post__in => array()` matching everything on old cores). **Litmus:** for every empty array or `null` passed to a third-party query/cancel/delete call, open the callee and find the line that decides what empty means; then pass the *documented* "any" form (here: hook only) rather than the narrowest-looking one. Pair with UC-3: a stub that records only some of the parameters cannot see a wrong value in the others.

### RI-31: A public filter's argument types are set by its first call site — a second call site that passes a different type breaks every callback written against the first
`apply_filters( OURS, $value, $order_reference )` from order creation hands callbacks a `WC_Order` (WooCommerce) or an `int` (EDD); the backfill route then applied the same filter with the request's raw **string** id for both platforms (#243, R33), so a site callback doing `$order->get_billing_country()` — correct on every order ever created — fatals on the one path a guest can trigger, and the plugin's own script triggers it on every receipt that needs a backfill, so the attribution the route exists to rescue is silently lost. A docblock saying `@param mixed … object or id` keeps the letter and promises nothing. **Litmus:** grep every `apply_filters( <our constant>` and tabulate the runtime type of each argument per call site and per platform; the table must have one type per column per platform, and the docblock must state it. RI-25 is the mirror image (their filter, our arguments); this is ours.

### RI-32: The second writer to a sanitized store is the one that skips the sanitizer
A class that cleans on write usually does it in one helper on the main write path (`record()` → `clean_entry()` → `text()`); a later "small" writer that updates one field in place (`record_status()` writing `result`, #244) goes around it, and the class docblock's "only sanitized … go in" becomes untrue for one member. The value was Google's, not an attacker's, and every reader was a text node, so it rated hardening only — but the shape is exactly how a real sink gets its first unsanitized member. **Litmus:** for each option row / meta / log the plugin writes, list every method that calls `update_option`/`save()` on it and confirm each routes text through the same helper; a writer that caps with `mb_substr` but never calls the helper is the tell.

### RI-33: An untyped sub-schema is a schema core cannot validate — and it says so with warnings, not with a refusal

`WP_Ability::execute()` validates an ability's **output** against its `output_schema` (`rest_validate_value_from_schema( $output, $schema, 'output' )`), and that validator recurses into every property present in the value, every array item and every `additionalProperties` schema it can reach. A sub-schema written as `array()` — meant as "any value" — has no `type`, so the validator calls `_doing_it_wrong()` ("The "type" schema keyword for output[…] is required"), then reads `$args['type']` anyway at three more sites (PHP 8: "Undefined array key" E_WARNING each), falls through to `$is_valid = true`, and the call succeeds. Measured on trunk's real `rest-api.php` for `get-settings` with `include_schema: true`: **414 E_WARNINGs + 276 `_doing_it_wrong`** per call (3 + 2 per field × 138 fields). On production the warnings reach the PHP error log; nothing reaches the JSON body (core forces `display_errors` off for JSON requests), which is exactly why it is invisible from a client and from a green suite (the tests stub WordPress).

The second half is the published contract: `wp_prepare_json_schema_for_client()` recurses only into children that are *not* numeric arrays, and `wp_is_numeric_array( array() )` is true, so the empty array is left alone and serialises as `"default": []` — an invalid JSON-Schema fragment (a schema must be an object), one level below the `properties` case U155 / `d2f8d6c` already fixed for the no-input abilities.

- **Litmus:** walk every `input_schema` and `output_schema` recursively (properties / patternProperties / items / additionalProperties) and require a `type` (or `anyOf`/`oneOf`) at every node. `tests/unit/Abilities/ContractTest.php` should carry that walk; its per-property loop at the top level cannot see this.
- **The fix shape:** a value that genuinely may be anything takes a multi-type `type` (`array( 'string', 'integer', 'number', 'boolean', 'array', 'object', 'null' )`); `rest_handle_multi_type_schema()` picks the best type and every real field default validates (probe-verified). Omitting the key is also warning-free but drops the property from the published list.
- **Seen:** #251 (2026-09-22, Low, `SettingsAbilities` `fields[].default`), the only untyped node in the eleven-ability catalogue at the time. Fixed the same day: typed, and `ContractTest::test_every_sub_schema_of_every_ability_carries_a_type` now walks every schema recursively.

### RI-34: A third-party exclusion or allow-list that matches OUR literal stops matching the day that literal becomes an option
A pattern list handed to somebody else (`rocket_excluded_inline_js_content`, a minifier
exclusion, a cache rule, a CSP hash list) that names a string we used to hardcode is a
second reader of that string. When RI-14 turns the literal into a configured value, the
code that pushes to the configured name is fixed and the exclusion that named the literal
is silently narrowed — no error, no failed test, the block just loses its protection on
every site with a non-default value (#325: the consent block after the #269 fix). When a
literal becomes configurable, grep for the literal **as a string in every list we emit**,
not only as an identifier in our code; and prefer a fixed, plugin-owned token (a marker
we always emit) over the user-chosen value as the list entry — a user value can be a
single letter or contain a regex metacharacter, and the matcher's semantics are theirs.
Ledger (re-derive): `rocket_excluded_inline_js_content()` is the one such list today.
...
- **An authorization gate is the mirror image (2026-09-23, R36, #289).** `get_http_origin()` also ends in `return apply_filters( 'http_origin', … )`, and the same-origin gate of the guest beacons reads it. The OR-floor above is the wrong shape there: for a *grant* decision the local read must be ANDed (`$own_check && helper()`), or the raw `$_SERVER` value read directly, because a site callback can only ever *widen* a grant. Rate it on the callback's owner (site-installed, A4-level) and measure the corpus both ways.
...
- **Per column.** `ContainerRows` defines four patterns, and since #60/#61 the sink applies every one of them: `container_environment()` re-checks `AUTH_PATTERN`/`PREVIEW_PATTERN` (`ContainerCode.php:791-792`), `container_path()` the path pattern, `container_domain()` FILTER_VALIDATE_DOMAIN. If a column has a pattern, the sink applies it. *(This bullet said "enforced only on save" until 2026-09-02 (R29) — it had gone stale against its own fix and misled a draft; a pattern entry describing a fixed defect must be updated in the fixing change.)*
...
### PA-21: A plugin-wide collector that keeps a per-module map ⭐

**Found:** 2026-09-21 (Abilities API phases 1b/1c, a maintainer question after phase 1). `Registrar::providers()` hardcoded the list of ability providers, and `StatusAbilities::modules()` hardcoded which module id has which master switch and which host-plugin version constant. Both were plugin-wide classes holding per-module facts: a third-party module registered through `gtm4wp_register_modules` could not contribute an ability and always reported `enabled: null`, and every new built-in module would have needed an edit in a file that is not its own. Neither failure raises an error or fails a test — the map simply does not know the module.

**The rule:** a plugin-wide surface that spans every module — the ability `Registrar`, the status/settings/site-health abilities, the Site Health Info section, the settings REST controller — is a *collector* and a thin adapter. Every module-specific fact it registers or reports is contributed by the module, in one of two ways:
- through an opt-in interface on the module's `AdminSchema` (`AbilitiesInterface`, `StatusInfoInterface`, `SiteHealthInfoInterface`, `PanelSchemaInterface`, `DocumentedSchemaInterface`), walked from the `Registry` with `instanceof` — never `method_exists()` — behind a `class_exists()` guard on the schema class, so a third-party schema written against an older build contributes nothing and does not fatal;
- or through a class inside the module that the collector calls explicitly (`Container\StatusReport`, `GoogleDataManager\SiteHealth`).

A feature-scoped ability lives in its module (`src/Modules/<Name>/Abilities.php`). The collector itself stays plugin-wide when its answer genuinely spans every module (`get-settings` is every module's options, `get-status` is "call this first"); that is a reason to keep the *ability* central, never a reason to hold module facts in it.

**Litmus:** in a class under `src/Abilities/`, `src/Admin/` or in `src/Plugin.php`, is there an array literal, `switch`/`match` or constant name keyed by a module id, or naming one module's option constant, version constant or class? Then either the module owns it (interface or module class) or the review records why the framework cannot express it. Regression: the third-party-module cases in `RegistrarTest` and `StatusAbilitiesTest` (`Admin\UndocumentedThirdPartyModule` plus the `*ThirdParty*` fixtures under `tests/unit/Abilities/`).
...
>
> **Third recurrence, and the rule for it: a NEW bundle ships the guard from birth (2026-09-02, #218).** `gtm4wp-edd.js` landed as a complete, well-tested tracker with no guard at all — probe-confirmed double-push (2 loads → 1 click → 2 add_to_cart) — and the review's own drafted fix then mirrored the `gtm4wp-woocommerce.js` counter-example shape, which only a verifier reading THIS entry's litmus caught. Two lessons: the pre-flight question for any new bundle is this litmus, not the sibling that happens to be open in the editor (the WC boot-entry guard is safe only because #82 also moved WC's state to `window` — copying half of an asymmetric fix recreates #82); and the suite consuming the bundle must clear the flag in `beforeEach` once the guard exists, or every test after the first boots into a no-op.
...
- **When you port another plugin's request-resolution chain, upstream's RESOLUTION and its AUTHORIZATION are usually separate halves in separate files — and porting only the resolver silently drops the gate (2026-09-02, #217/#219).** Upstream code that turns request parameters into a record routinely splits "which record is this?" (a resolver that checks nothing) from "may THIS caller see it?" (a can-view/hash check that runs later, sometimes in a different file, sometimes only in the rendering layer). A port that mirrors the resolver line-by-line reads as faithful — the fallback chain matches, the parameter names match — while the enforcement the resolver *feeds into* upstream never came over. The tell is a docblock asserting parity with the upstream chain: treat "mirrors X's own fallback chain" exactly like PA-10 treats a `__return_true` docblock — enumerate BOTH halves of the upstream flow (the resolver AND every check between it and upstream's actual output) before accepting the parity claim, and re-derive what upstream does with each request parameter the port reads (a parameter checked only for *presence* in the port may be a *verification value* upstream). This is also RI-18's shape ("a docblock asserting a security property is a claim to test") and the WC module's recorded parity principle ("upstream parity in both directions — publishing more than upstream is the real failure") applied at porting time. This surface is invisible to the route-shaped sweep: a conditional-tag-gated dataLayer emitter has no `register_rest_route`, no `permission_callback`, and no inventory row.
...
- **Fail closed.** `bash <(git show "$REF:$SRC")` reads beautifully and **fails open**: an
  ...
  not a fix.* **One exception, stated so it is not re-flagged (R35, #268): a hook that only
  ever ADVISES and never gates (`gtm4wp-prose-budget`) may fail open — silence there is a
  lost report, not lost enforcement. The fixed-ref half still applies to it in full.**
...
- **The out-of-repo scope also goes stale on its own CLOCK, not the repo's (#224, 2026-09-02).**
  Every staleness trigger this system had was repo-keyed (commit ranges, the ~4-week sweep
  rule), so a marketplace plugin update that landed the same morning as a review invalidated
  the plugin-scope ledger *while the review correctly followed the rules*: claude-security
  0.10.2.3 → 0.11.0 changed the agent count and the hook trigger set, and the inherited
  figures were wrong by evening. The trigger for the plugin leg is
  `installed_plugins.json`'s `version`/`lastUpdated` **read on disk every run**, plus the
  active-tree digest compared against the recorded pin — measured **0.106s**, so cost never
  justifies skipping it. Digest counting rule: exclude `./.in_use/*` (PID false drift, R26)
  **and `.orphaned_at`** (the updater stamps it into the superseded version's tree at update
  time, so a digest of a non-active version silently differs for non-content reasons) **and
  `*/__pycache__/*`** (R33/#247: the plugin's own scripts compile `.pyc` files into the active
  tree the first time they run, which moved the digest of an unchanged 0.11.0 — the `.pyc`
  file-count trap two bullets up is a digest trap too).
...
### PA-20: A new stored asset class invalidates threat-model scope claims silently

**Found:** 2026-09-03 (Review 30, #230). The threat model's out-of-scope list said "the
plugin stores no credentials" — true when seeded (2026-07-17), false the moment the
google-auth module's `KeyVault` landed, with nothing to flag the expiry: the feature
commit has no reason to touch `.security/threat-model.md`, and no sweep reads that file
against the tree.

**The rule:** a scope claim ("we store no X", "no route returns Y", "nothing runs on
Z") is a premise about the codebase, and premises age like the recorded decisions the
adjudication stage re-derives. When a change introduces a new *kind* of stored asset —
not another instance of an existing kind — the same change (or the review that first
covers it) must (1) re-read the threat model's in/out-of-scope lists for premises the
asset falsifies, and (2) add the asset's rating vocabulary, because the actor ladder
was built for the assets that existed at seeding time (a stored third-party credential
pivots off-site, which no A0–A4 rung and neither D-actor expresses).

**Litmus:** does the diff add a new option row / table / file store whose content class
(credential, secret, PII aggregate, executable) the threat model never names? Then the
threat model is part of the change's blast radius.
...
- **Re-derived 2026-09-22 (R34)** (rotation; `ScriptTag.php` in range for comment compaction only, verified code-identical): caller ledger **12**, same distribution (ContainerCode 8 / UserEvents 2 / PageDataLayer 1 / PurchaseTracking 1), identical at the base sha. **Counting trap, now stated:** `ContainerCode::the_tag()` calls `print_markup_block(` with its argument on the NEXT line, so a single-line regex such as `print_(script|markup)_block\(\s*[^)]` returns **11** — the 12th is real. Grep the bare call name and subtract the two definitions and the prose mentions instead.
...
**Re-derived 2026-09-20 (R33) — the gate moved, the suppression did not widen.** The Origin/Referer/nonce reasoning was extracted verbatim from `PageDataLayer` into `src/RequestOrigin.php` (`is_same_origin_request()`, `has_rest_nonce()`) so that the Data Manager **attribution-backfill** route could share it. All three conditions still hold for the two beacons (no parameters, marker consumed up front, gate intact). The backfill route is **not** an FP-5 route and must not be read as one: it takes the record id from the *request* and answers PA-10 with proof of purchase (WC order key via `hash_equals`, EDD lookup *by* payment key), and its safety rests on the write-only-if-absent rule — a different mechanism with its own inventory row. Sharing the gate class does not share the waiver.
...
| 2026-09-03 (Review 31) | Reviewed `16f67c0..f0b3667` (3 commits — the Google Data Manager phase-2 module: destinations table + A4 validateOnly test route, health-record skeleton, the `columnChoices` panel-data seam, `WpTransport` explicit-port rejection). **3 Low (#232–#234), 0 Medium+** — the new surface conforms to PA-1/PA-2/PA-10/PA-13 sink-for-sink; the fixed-endpoint probe sits behind the transport allow-list; custody holds on every new response path. Added **RI-28** (an "empty/untouched row" predicate must match the state the UI actually creates — a seeded select-column default made the sanitizer's silent-drop path unreachable and the drop test, missing the seeded cell, passed forever; #232) with its Quick Index row. Adjudication: 3 drafts / 3 verifiers, 0 mechanisms refuted, **1 reach claim refuted** (#232 drafted Medium on a "blocks the whole save" premise; the save path is per-field, so it shipped as Low with the measured blast radius), 5 collaterals — including a stale panel result suppressing the real failing-health warning (#234's own root cause, adjacent symptom) and the #233 verifier narrowing #229's residue to `Transport::get()`. **FP-4 re-derived** (rotation, R30's nomination): the AMP passthrough still hands the raw compiled data layer to `config_data['vars']` with no pre-escaping, delegated sink verbatim, AMP untouched in range; **FP-1 additionally re-derived** (12 callers, same distribution, no caller in range). **Next review re-derives FP-5.** |
| 2026-09-03 (Review 30) | Reviewed `91ece19..ea7ece7` (5 commits — the whole google-auth module: encrypted service-account key custody, token minting, 4 A4 REST routes, the first custom settings panel; plus the Infection toolchain). **6 Low (#226–#231), 0 Medium+** — the new surface conforms to PA-1/PA-2/PA-10/PA-13 sink-for-sink, custody is pinned by test on all four serialization paths, and the SSRF guard is a transport-seam host allow-list. Added **PA-20** (a new stored asset class invalidates threat-model scope claims silently) from #230, fixed in-session: the "stores no credentials" out-of-scope premise plus the missing off-site-pivot rating vocabulary, now in the threat model with the DB-stored-salts caveat. Adjudication: 6 drafts / 6 verifiers, 0 mechanisms refuted, 1 cost-framing corrected (#231's "wp_cache locking" is no cross-request mutex on default installs; honest menu is custom table / raw conditional UPDATE — verified against current core source: `update_option` has no old-value predicate, `add_option` is a blind upsert), 5 collaterals. **FP-2 re-derived** (rotation): both encode legs still full-hex, sink still `wp_add_inline_script`. **Next review re-derives FP-4.** |
...
| 2026-09-20 (Review 33) | Reviewed `f0b3667..78ec702` on `master` (80 commits — Data Manager phases 3+4: attribution capture, backfill route, consent policy, the refund send lane, queue, status polling, send log + replay, Site Health, privacy export/erasure). **9 Low (#240–#248), 0 Medium+, 0 security.** Added **RI-30** (an empty collection passed to a third-party API is a predicate, not an absence — `as_unschedule_all_actions( $hook, array(), $group )` cancels nothing, #240), **RI-31** (our own filter's argument types are set by the first call site; a second site passing a different type per platform breaks callbacks, #243), **RI-32** (the second writer to a sanitized store skips the sanitizer, #244), each with a Quick Index row. **PA-12** gained clause (d) — a nonce baked into a page the platform itself marks `DONOTCACHEPAGE` on the same predicate is fine (re-derived from WC 5.0.0/trunk and EDD 3.0.0/3.7.0 source for #245; coupling registered as U141). **PA-19**'s digest rule now excludes `*/__pycache__/*` (#247: the plugin's own scan compiled `.pyc` files into the active tree and moved the pin with no update). **FP-5** re-derived on its extraction into `RequestOrigin` and scoped explicitly *away* from the backfill route (different mechanism: request id + proof of purchase, write-only-if-absent). **FP-2** re-derived (rotation) — holds. Fix session `ba57575` the same evening: #240–#244, #249, #250 closed, 0 claims changed shape. |
| 2026-09-22 (Review 34) | Reviewed `ba57575..f60351d` on `feature/abilities-api` (25 commits — the WordPress Abilities API layer, eleven abilities; 8 comment-compaction commits verified code-identical). **10 Low (#251–#260), nothing above.** Added **RI-33** (an untyped sub-schema: published as `[]`, and core's validator warns three times per value and passes — measured 414 + 276 per `include_schema` call against trunk's real `rest-api.php`). Extended **RI-5** to ability schema descriptions (39 untranslated; core and WooCommerce wrap every one). **FP-1 re-derived** (12 callers; the multi-line `print_markup_block(` counting trap written into the entry). The adjudication stage refuted one mechanism half (#252's environment tokens are in every page's public loader URL), two dispositions (#252 "needs design" → doc fix once masking was measured to empty the container table on import; #257 "unmeasured" → 0.30 ms vs 0.14 ms), a hedge (#256), and corrected three recommendations and two consequence statements. #260: a sweep figure ("0 `global` statements") was a shell-quoting artefact — a double-quoted `\$` is an end anchor. |
| 2026-09-23 (Review 35) | Reviewed `439d057..be96c49` on `master` (10 commits — the Site Health generalisation with `SiteHealthRows`/`SiteHealthTests`/`SiteHealthTestsInterface`, the pending-purchase re-check, the EDD 3.7.1 receipt-hash delegation, the prose-budget Stop hook; one compaction commit verified code-identical). **8 Low (#261–#268), nothing above; 1 draft (#263) turned into a by-design note by the verifier.** Extended **PA-14** with the advisory fail-open exception (#268: a hook that never gates may fail open; the fixed-ref half still applies), **PA-21** with the external-identifier corollary (#266: a collector that turns a module id into an HTML id / selector owns the character-set and collision rules — the registry validates nothing) and listed `SiteHealthTestsInterface` among the opt-ins, **RI-15** with the unit rule for status rows (#261: the label carries the unit the code applies). FP-2 re-derived (holds; full hex set present on an inline-script sink). |
| 2026-09-23 (Review 36) | Reviewed `bc6dc70..419c534` on `master` (6 commits, 3 production files: the WC 11.1.2 header, the `$gtp4wp_*` globals removed, the confirm beacon's state-free pending early-return) — a near-empty diff, so the **deep mode** ran: five parallel re-audits of the oldest-reviewed groups + all nine sweeps. **1 Medium (#269), 42 Low (#270–#311).** Extended **RI-14** twice (the PHP writers are the same class as the JS default-name literal — #269, the consent default block; a cookie's identity is name + Domain + Path — #270), **RI-26** (an authorization gate is the mirror image: AND-floor or a direct read, never the OR-floor — #289), **RI-19** (`function_exists()` conflates "file loaded" with "feature initialised" — #282) and **RI-16** (the inverse spelling: a truthy failure value from DOM named-property shadowing — #280). FP-4 re-derived at amp-wp source, FP-5 in code. Adjudication: 43 drafts, 26 to 15 verifiers, 1 mechanism refuted (the admin-ajax marker path — the boot split closes it), 12 recommendations refuted or corrected, 4 measured to completion. The command's PATCH-verifier bullet corrected: the Agent tool's `isolation: "worktree"` lands inside the repo (#311). |
| 2026-09-23 (Review 37) | Reviewed `419c534..cdb8fbc` on `master` (5 commits, 30 production files — R36's entire fix session, read as new code against each finding and the premise its design rests on; `PageVariablesModule.php` in full). **11 Low (#312–#322), nothing above.** Extended **RI-13** (a fix's own hunk is where the straggler hides — #312, `visitorIP` left as context beside four omitted siblings; a conditional tag guarantees `isset`, not a class — #315) and **RI-14** (a removed literal must not come back as a parameter default — #317). FP-1 re-derived (12 callers), FP-2 (file in range). Adjudication: 4 of 11 drafts to verifiers, 0 mechanisms refuted, 2 mechanism statements and 1 reach corrected, 1 recommendation half refuted (#313 suppress — its own remedy swaps the warnings), 1 cost corrected (#314), 1 disposition refuted (#312 needs no new decision). |
| 2026-09-23 (Review 38) | Reviewed `2f53e06..245e2e8` on `master` (3 commits, 1 production file — the Google tag developer-ID push in `ContainerCode::header_top()`; the base fix commit re-read as new code) — a near-empty diff, so the **deep mode** ran: all nine sweeps, six oldest cells, FP-4 re-derived. **2 Low (#323, #325) + 1 observation (#326), nothing above; no security finding.** Added **RI-34** (a third-party exclusion list naming a literal we later made configurable is silently narrowed — #325, the WP Rocket exclusion after the #269 consent-shim fix). Adjudication: 4 drafts, 0 mechanisms refuted, 2 recommendations refuted (#323 missed the `Transport` fake, corrected and re-measured green; #325 the configured-name append), 1 disposition refuted (#306 "parameter kept" now carries a measured removal cost), 1 draft withdrawn as a duplicate of #306. |
