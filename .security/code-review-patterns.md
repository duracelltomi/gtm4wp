...
- **FP-5** — a guest-facing *frontend* REST mutation may use nonce + strict session-ownership-scoping instead of a capability check (PA-1), when it only ever affects the caller's own session-resolved record, is idempotent, and is non-sensitive. `PageDataLayer::confirm_pending_purchase_tracked()` (#398); the public visitor-data GET endpoint is the sibling.
