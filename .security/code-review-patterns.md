...
- **PA-19** — a permission ledger is only as wide as the SCOPES it enumerates. The enforced allowlist is merged from user / project / local scope (plus per-agent frontmatter, which *restricts* rather than *grants*); this system read only the two project-scope files for 21 reviews while a user-scope dispatcher sat unread outside the repository (#162). Enumerate scopes, then files.
...
### PA-19: A permission ledger is only as wide as the SCOPES it enumerates, and the enforced set is assembled from more than one
...
**What R14 actually corrected was the packaging, not the entry.** The body above has said "resolved" since R4; the **Quick Index** row went on presenting it in the present tense under "do NOT flag", and several coverage notes still carry a "bar the pre-existing FP-3 `$echo` warnings" caveat as though it were current. So the stale claim lived in the summary that gets read, not in the entry that does not. **When you resolve an FP, fix its index row and grep the checklist for prose that still quotes it** — a resolved entry whose index row reads live is indistinguishable from a live one at the only moment anybody looks.
