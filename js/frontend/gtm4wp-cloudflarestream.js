...
function gtm4wp_initCloudflareStreamTracking() {
	...
	gtm4wp_cloudflarestream_frames.forEach( function ( stream_frame ) {
		...
		const gtm4wp_onStreamPercentageChange = function () {
			...
			if (
				typeof gtm4wp_cloudflarestream_percentage_tracking_marks[
					videoid
				] === 'undefined'
			) {
				gtm4wp_cloudflarestream_percentage_tracking_marks[ videoid ] =
					[];
			}
			...
			for (
				let i = 0;
				i < 100;
				i += gtm4wp_cloudflarestream_percentage_tracking
			) {
				if (
					videoPercentage > i &&
...
if ( document.readyState === 'loading' ) {
	window.addEventListener(
		'DOMContentLoaded',
		gtm4wp_initCloudflareStreamTracking
	);
} else {
	gtm4wp_initCloudflareStreamTracking();
}
