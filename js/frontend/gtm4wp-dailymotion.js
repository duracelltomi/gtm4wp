...
function gtm4wp_initDailymotionTracking() {
	...
	gtm4wp_dailymotion_frames.forEach( function ( dailymotion_frame ) {
		...
		const gtm4wp_onDailymotionPercentageChange = function () {
			...
			if (
				typeof gtm4wp_dailymotion_percentage_tracking_marks[
					videoid
				] === 'undefined'
			) {
				gtm4wp_dailymotion_percentage_tracking_marks[ videoid ] = [];
			}

			for (
				let i = 0;
				i < 100;
				i += gtm4wp_dailymotion_percentage_tracking
			) {
				if (
					videoPercentage > i &&
...
if ( document.readyState === 'loading' ) {
	window.addEventListener(
		'DOMContentLoaded',
		gtm4wp_initDailymotionTracking
	);
} else {
	gtm4wp_initDailymotionTracking();
}
