...
* Added: modern React-based settings screen with left pane navigation, tabbed option groups, option search and inline per-field validation. The screen is usable on a phone: the title and the "Export settings" / "Import settings" / "Save changes" buttons stack instead of running off the edge of the screen, a row of option group tabs that is wider than the screen scrolls sideways instead of being cut off with the last tabs unreachable, and the container table turns into one card per container with every column labelled above its own field, instead of six columns squeezed into the width of a phone.
