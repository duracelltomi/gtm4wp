...
* Changed: `visitorEmail`, `visitorEmailHash`, `visitorUsername` and `visitorRegistrationDate` are omitted for a logged-out visitor or an empty value instead of being reported empty. `visitorEmailHash` is now lower-cased and normalised the way Google matches user-provided data, so it changes for mixed-case and Gmail addresses.
