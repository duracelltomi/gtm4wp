...
* Added: the plugin registers **abilities** with the WordPress Abilities API (WordPress 6.9 and newer; on an older WordPress nothing changes), so an AI assistant connected to the site - Claude Code or Claude Desktop, Cursor, Codex, or any other client of the WordPress MCP Adapter plugin - can read how Google Tag Manager for WordPress is configured and help find out why tracking is not working, without anyone pasting settings into a chat. Four read-only abilities under the `gtm4wp` category: `gtm4wp/get-status` (which containers load, with any wp-config.php override applied, where the container code is placed, the data layer variable name, which modules are available and switched on, which supported plugins are installed, and the same configuration problems the admin notices report), `gtm4wp/get-settings` (every option with its current value, filterable by section or by option key, optionally with each option's label, description, type, default, allowed choices and documentation link), `gtm4wp/get-site-health` (the plugin's Site Health status test and its Info section) and `gtm4wp/get-google-data-manager-log` (the recent server-side sends of the Google Data Manager integration and what became of them). Every ability requires the same capability as the settings screen (`gtm4wp_admin_page_capability`, `manage_options` by default) and is listed on the `wp-abilities/v1` REST routes as well. What an ability returns is limited to what the settings screen and the Site Health Info section already show to that capability: no keys, no service-account addresses, no raw Google error text, no visitor data. Two filters opt out: `gtm4wp_abilities_enabled` (return false to register nothing) and `gtm4wp_abilities_allow_write` (return false to keep the surface read-only once abilities that change settings exist; none does in this version). An assistant reaches the abilities through the WordPress MCP Adapter plugin or any other Abilities API client - the plugin deliberately ships no MCP server of its own, so one connection serves every plugin on the site. **Experimental.**
