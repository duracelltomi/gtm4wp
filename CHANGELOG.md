...
* Fixed: an Easy Digital Downloads category name containing an ampersand reached the data layer as `Shirts &amp; Ties` rather than `Shirts & Ties`, and that is the literal string GA4 reported. WordPress encodes a term name when it is saved, so what the plugin read back was the encoded form; it is decoded once now, at the point the name is read, exactly as 2.0.2 does for WooCommerce categories and brands, and the value still reaches the page through the same JSON encoder as before, so nothing about the escaping of the script changes.
* Changed: the page variables that carry term names - `pagePostTerms` and `pagePrimaryCategoryName` - now report them as they were typed as well, so a category named "Shirts & Ties" arrives as exactly that rather than as `Shirts &amp; Ties`, the same string the e-commerce items have reported since 2.0.2. One decode, shared with the product data, so a GTM trigger matches the same text whichever variable it reads. If a trigger of yours matched the encoded form on one of these two variables, it needs the plain text now; slugs (`pageCategory`, `pagePrimaryCategory`) were never encoded and are unchanged.
...
* Added: **attribution capture** (experimental, off by default), on a new **Attribution capture** tab of the Google Data Manager section: the setting that lets the plugin store with every new order what is needed to match a later server-side event - a refund, above all - to the original purchase in Google Analytics: the Analytics client and session IDs, the Google Ads click IDs (`gclid`, `gbraid`, `wbraid`) of the visit, and the consent state at the time of the order. Because the IDs are read through the official Google tag API (`gtag('get', ...)`) rather than by picking Google's own cookies apart, the setting needs at least one destination above whose measurement ID belongs to a Google Analytics 4 tag that really fires in your container - the checkbox stays disabled until a destination exists, and the option description says so. The lookups are queued once per page view and answered whenever the Google tag loads, so a container that brings Google Analytics in a few seconds late is served just as well as one that loads it immediately; if no such tag ever fires, nothing is stored and nothing is invented. Everything captured is held in memory until the visitor's consent state allows storing it: the Analytics IDs need analytics storage and the click IDs need ad storage to be granted, a site running no consent mode at all stores normally, and a banner answered on the landing page itself still saves the click ID that page arrived with. The consent state itself is written in every case, denial included - it is the record of the choice. The visitor's answer is read from the Google tag's own consent state, so a consent tool that lives inside your GTM container - a consent-mode tag template rather than a script on the page - is understood exactly like one that announces itself on the page; those are invisible to the page itself, and reading only the page would mean both storing data for visitors who refused and discarding it for visitors who agreed. Consent is honoured in both directions: if the visitor later withdraws it, or if a consent tool that loads a moment after the page finishes answers no, whatever had already been stored is removed again rather than merely left alone. The values live in two first-party cookies of the site's own; no third-party cookie is involved and no Google cookie is ever read or written. On the server they are read back the moment an order is created - on the WooCommerce classic checkout, on the block checkout and on Easy Digital Downloads alike - and stored in the order's own meta under namespaced `_gtm4wp_*` keys. Every value is checked against a strict format and dropped if it does not fit, and anything that was never captured is left out of the order rather than stored as an empty value. Site code can supply or override the consent state per order through the new `gtm4wp_gdm_order_consent` filter, which is the way out for consent tools that keep the visitor's choice inside the GTM container where the page never reflects it; the filter has the last word on every path that stores a consent state, and the server drops the identifiers that the resulting answer does not allow rather than trusting the browser to have done it. A buyer whose only page view was the checkout can place the order before Google has answered, so the confirmation page tries once more and sends what it resolved to the order it belongs to: the page carries that request only when the order really is still missing its attribution, the request has to come from a page on this site and carry the order's own proof of purchase - the WooCommerce order key or the Easy Digital Downloads payment key, the same secret that authorizes showing the receipt, or on EDD's own receipt links the verification hash those links carry instead of the key, so the page never prints a secret its address bar does not already hold - and it can only fill in what is empty, so nothing can overwrite attribution that is already there; a consent denial already recorded with the order keeps the identifiers it forbids out as well, whatever the confirmation page reports later. The plugin also keeps a small counter of how many recent orders carried attribution, so that a mistyped measurement ID or an Analytics tag removed from the container becomes something the plugin can report rather than a mystery discovered weeks later. All of this is wired into WordPress' own privacy tools on both platforms: an export request lists what is stored against that person's orders in plain words, and an erasure request removes it, in a single write per order however many fields were stored. That matters more than it may sound, because uninstalling the plugin deliberately leaves order meta alone - sweeping every order of a store is expensive, and the existing purchase-tracking flag works the same way - which makes the erasure request the way to remove one person's data.
* Added: **server-side refund events** (experimental, off by default), on a new **Sending events** tab of the Google Data Manager section - the signal browser-side tracking can never report. A refund is issued in the store admin, where no page is loaded and no tag fires, so Google Analytics keeps counting revenue that was given back. With this on, every refund issued in WooCommerce or in Easy Digital Downloads is sent to each configured destination as a GA4 `refund` event, matched to its original purchase by the same transaction ID the purchase event reported and by the client ID the attribution capture above stored with the order. Every refund reports its own amount, the items it covers and the shipping and tax it returned - a refund of the whole order is one that lists every line - so item-level refund metrics work and Google Analytics stops counting refunded shipping and tax as revenue it kept. That is one shape for every refund on purpose: the acceptance run measured that a whole-order refund sent as the transaction ID alone, the form Google's older gtag guidance described, is accepted and applied by Google and then attributed a refund amount of zero, while refunds carrying their amount are attributed correctly. It also means a sequence of partial refunds simply adds up, each slice reported once, with no special case for the one that completes the order. One known limitation, stated on the option itself and being taken up with Google: when an order is refunded in several steps, Google Analytics has been observed to count only the first step even though it accepts and confirms every one; each step is still sent, so nothing has to be resent if that changes. Refunded line items are built by the very same builder as the purchase items, so their `item_id` matches whatever your store settings make it (a SKU, a product ID, a master-language product) rather than being derived a second time - and they carry the same name, brand, variant, categories, per-unit discount and affiliation the purchase sent, because Google Analytics does not look those up from the purchase: a refunded line without them is reported as "(not set)" with its amount attributed to no product. A variation is identified and described exactly as at purchase (its own ID or SKU, the variant string, the parent's categories and brand), and a grouped product's children are ordinary lines on both sides. Sending never happens inside the request that issued the refund: the refund is queued and sent about a minute later in the background, through Action Scheduler where the store provides it - WooCommerce always, Easy Digital Downloads since its version 3.6.5 - and through WP-Cron anywhere else, so every store is served the same way; queued jobs are removed again on uninstall on either backend. A rate limit, a network failure or a Google-side outage is retried on a widening schedule from one minute out to twelve hours; a request Google refused as wrong is not repeated. Where several destinations are configured, each one reports its own outcome and only the ones that failed are retried, so a destination that already accepted the event never receives it twice. Every refund is sent exactly once, remembered on the refund itself, so several partial refunds of one order each send once and a repeated background job sends nothing. Nothing is ever guessed: an order with no stored client ID, an order whose consent rule does not allow the transfer, a store with no destination configured - each of those is skipped, and the reason is recorded rather than left as a silence. Where the missing client ID is itself the consequence of the buyer refusing analytics storage, the list says so instead of reporting the missing ID on its own, because the two are answered differently: one is a setup question, the other is the visitor's decision and no setting can override it. And where a store has no destination configured at all, that is what every skipped refund reports, ahead of anything particular to the order: it stops every send on the site and is fixed once, so it is the answer worth giving first. The consent rule from the Attribution capture tab is what decides, and where it applies, analytics storage has to have been granted at the time of the order; a denied or simply unknown answer refuses the send, which also protects the data, because a client ID handed out while analytics storage was denied can be a throwaway one that would never match anything. Where the buyer's ads consent was recorded, it travels with the request. Site code gets the last word through the new `gtm4wp_gdm_refund_event` filter, which receives the assembled event together with the platform's own refund and order objects and can change it or cancel the send.
* Changed: the settings screen now says **Unsaved changes** next to the Save button while anything is waiting, and the browser asks before a tab is closed or reloaded with unsaved edits on it. Nothing on that screen is written until Save, and some edits look final enough to be taken for done - a removed table row above all.
* Changed: removing a row from a settings table - a container, a Google Data Manager destination - now asks first. The trash icon turns into a Remove/Cancel pair for that row instead of deleting it under the pointer, and a row nobody has typed into is still removed straight away, because there is nothing to lose and a question asked over nothing only teaches people to click through it.
* Added: a **Recent sends** list under the destinations table, and two new rows in **Tools -> Site Health**. The list appears once a server-side send is turned on, and stays as long as it has something to show, so a site that sends nothing is not given a permanently empty block to read. It shows what the plugin has sent to Google from the server and what became of it, including every case where it deliberately sent nothing and why - a missing client ID, a consent answer that did not allow it, a destination that is not configured. Rows are colour coded so the ones that need attention are found without reading the whole list: red for an event that did not arrive, amber for one that was skipped, is being retried or came back with warnings, green once Google reports it applied, and plain text while Google is still deciding. The colour is never the only signal - the outcome and the explanation stay written out in words. A switch above the list hides everything Google accepted and applied, leaving the failures and the deliberate skips, and it names how many of those there are before you touch it. The list keeps the fifty most recent sends, and when it has to make room it drops routine sends before problems, so a store refunding a batch of orders does not push this morning's failure out of the list before anyone reads it. Once the cause of a failure is corrected, the refunds it cost can be sent again: one button queues every refund that can be sent again at once - counted as refunds, not rows, so a refund that failed six times over two destinations is one job - and each row has its own link for a single one. A refund is queued only for the destinations that never took it, so one that already accepted the event is never sent it twice - and once a refund has gone through, its older failed rows stop offering to send it again, and a refund skipped for a reason a setting can fix (no destination, an unrecorded consent answer, a cancelled event) can be queued again as well; one skipped because no client ID could ever exist for it cannot. Pressing Test on a corrected destination also ends its failure streak, so the site-wide notice goes as soon as the fix is proven rather than at the next customer's refund. Because Google decides asynchronously (an accepted request is processed anywhere between half an hour and a day later), the plugin asks back on Google's own published schedule and records the answer, so "Google took it" and "Google applied it" stay distinguishable. Site Health adds a status test that reports, as critical issues, a service account key that can no longer be read and a destination whose sends keep failing - both mean events the store is set up to send are being lost with every refund, and the second one already raises a red admin notice, so Site Health says the same thing - and, as a recommended improvement, the failure this feature would otherwise be silent about: orders arriving with no attribution captured at all, which is what a mistyped measurement ID or a Google Analytics tag removed from the container looks like. Its Info tab gains a section with the state of all of this, deliberately limited to statuses, counts, timestamps and short reason codes: no account address, no key material and no property IDs, so it stays safe to paste into a support thread. The section is the plugin's, not this feature's: any module can report rows into it by implementing the new `SiteHealthInfoInterface` on its admin schema, and site code can add rows through the new `gtm4wp_site_health_info` filter, under the same rule about what may appear there.
...
* Added: the plugin registers **abilities** with the WordPress Abilities API (WordPress 6.9 and newer; on an older WordPress nothing changes), so an AI assistant connected to the site - Claude Code or Claude Desktop, Cursor, Codex, or any other client of the WordPress MCP Adapter plugin - can read how Google Tag Manager for WordPress is configured, help find out why tracking is not working, and change a setting once you have confirmed it - without anyone pasting settings into a chat. Eleven abilities under the `gtm4wp` category, six of them read-only: `gtm4wp/get-status` (which containers load, with any wp-config.php override applied, where the container code is placed, the data layer variable name, which modules are available and switched on, which supported plugins are installed, and the same configuration problems the admin notices report), `gtm4wp/get-settings` (every option with its current value, filterable by section or by option key, optionally with each option's label, description, type, default, allowed choices and documentation link), `gtm4wp/get-site-health` (the plugin's Site Health status test and its Info section), `gtm4wp/get-google-data-manager-log` (the recent server-side sends of the Google Data Manager integration and what became of them), `gtm4wp/export-settings` (the settings as the JSON envelope the Export button downloads, never with a service-account key in it) and `gtm4wp/get-service-accounts` (the stored Google service accounts by id, label, status and last test - never their e-mail address, key id or key), and five that change something or contact Google. `gtm4wp/update-settings` changes only the options named in the call and saves them the way the settings screen's Save button does - every value through the same per-option sanitizer, a refused value reported and not stored, options that wp-config.php constants control left alone - and answers with the options that actually changed (a table option such as the container list is replaced as a whole, so its description tells the assistant to send every row with every column). Its description instructs the assistant to read the current values first, to show you the current and the new value of each option and to wait for your explicit yes before calling it, and it is marked destructive so an MCP client asks you before running it anyway. `get-settings` returns a fingerprint of the stored settings that the assistant passes back with the change; if anyone saved in between (the settings screen, an import, another assistant) the change is refused and nothing is overwritten. `gtm4wp/import-settings` is the Import button: it replaces every option from an export envelope, through the same checks and sanitizers as an uploaded file, and must be called with `confirm: true` after the assistant has exported the current settings and asked "Replace all settings on this site?". `gtm4wp/test-service-account` and `gtm4wp/test-google-data-manager-destination` are the Test buttons of the service-accounts panel and of the destinations table (a token minted with a stored key; a validate-only request for a stored destination named by its measurement ID - an unknown one is refused before anything is sent, and the assistant never supplies an account or property ID of its own), and `gtm4wp/replay-google-data-manager-refunds` is the "send again" action for failed or fixable refund events, also behind `confirm: true`; the three tell the MCP client that they contact Google. Every ability requires the same capability as the settings screen (`gtm4wp_admin_page_capability`, `manage_options` by default) and is listed on the `wp-abilities/v1` REST routes as well. What an ability returns is limited to what the settings screen and the Site Health Info section already show to that capability: no keys, no service-account addresses, no raw Google error text, no visitor data. Two filters opt out: `gtm4wp_abilities_enabled` (return false to register nothing) and `gtm4wp_abilities_allow_write` (return false to keep the surface read-only: the five abilities that change something or contact Google are then not registered and refuse to run). An assistant reaches the abilities through the WordPress MCP Adapter plugin or any other Abilities API client - the plugin deliberately ships no MCP server of its own, so one connection serves every plugin on the site. **Experimental.**
* Changed: the admin notices about a missing container ID, an incomplete environment configuration, a malformed `GTM4WP_HARDCODED_*` constant, a visitor IP header with no trusted proxies and an unusable data layer variable name now carry a separate "Open the setting" link after the message instead of linking a few words inside it. The checks behind the notices are shared with `gtm4wp/get-status`, so an assistant and the screen report the same problems.
...
* Fixed: `add_to_cart` and `remove_from_cart` never fired on a store whose product and cart blocks are built on the WordPress Interactivity API, which is the newer form of the WooCommerce blocks. Two separate causes, both fixed. On a product page WooCommerce renders the Add to Cart + Options block without the `cart` class it puts on the classic form, and the plugin looked for exactly that class before reading the product data, so a click was never reported even though the button carried the classes it expects. That page now reports the add once WooCommerce confirms that the item really reached the cart, so an add refused for being out of stock or by the block's own validation still reports nothing. Elsewhere, those blocks keep the cart in a private store that the plugin must not read, so the cart is read back from the WooCommerce Store API whenever WooCommerce announces that it changed, and the difference is reported as `add_to_cart` or `remove_from_cart` exactly as on a store using the older blocks. The extra request is only made for a visitor who already has a cart, and only when the cart actually changes. A variable product added from such a page is covered as well: the interactive form publishes no variation data the plugin could read, and the parent product's price would be the wrong number to report, so the event is completed from the cart line the add creates, which carries the finished item for the variation itself.
...
* Fixed: custom fields holding a structured value (a list or a nested structure rather than plain text) reached the `pagePostTerms.meta` data layer variable as an unreadable packed string such as `a:8:{s:8:"metadata";...}`. WordPress hands back the raw database column when all custom fields of a post are read at once, so anything a plugin stored that way - an SEO plugin's schema template, an Advanced Custom Fields repeater - arrived packed. No Google Tag Manager variable could read such a value, while the whole internal structure the plugin keeps behind that field was still published on the public page and added to the weight of every page view. Those values are now skipped; plain-text custom fields are unaffected. Two shape details follow from that: a custom field that stored several values of which only one is plain text now arrives as that single value rather than as a one-item list, so what your container receives no longer depends on what was dropped; and a post with no publishable custom fields at all leaves `pagePostTerms.meta` out of the data layer entirely instead of sending an empty one. **Check your Google Tag Manager setup if a variable or trigger expects `pagePostTerms.meta` to always be present.** Custom fields are now also checked with the WordPress `is_protected_meta()` function in addition to the leading-underscore rule, so a field that a plugin or your own site marks as protected is held back as well - previously only names starting with `_` were, which is why fields from some SEO plugins were published while the equivalents from others were not. The underscore rule still applies on its own, so a field whose name starts with `_` is never published even on a site whose code declares that field unprotected.
...
* Changed: on the order received page, the customer details in the `orderData` variable, the `new_customer` / `customer_type` signals and the Enhanced Conversions `user_data` block on the `purchase` event are now included only when WooCommerce itself would show that order to the visitor viewing the page. The `purchase` event, the order totals and the items are unchanged and still fire exactly as before, so nothing is lost for a buyer arriving from checkout. WooCommerce's own `woocommerce_order_received_verify_known_shoppers`, `woocommerce_order_email_verification_grace_period` and `woocommerce_order_email_verification_required` filters are all honoured, so a store that has relaxed any of these checks keeps the matching behavior here - and a callback you already have receives the same arguments WooCommerce passes it, so it keeps working unchanged. The decision follows your WooCommerce version: where WooCommerce has these visitor checks, the plugin mirrors them (asking WooCommerce's own helper where one exists, deriving the same answer from the order age and your filters where it does not), and on older releases without any such check - below WooCommerce 7.9, which shows the order to anyone opening the confirmation link - the data layer keeps matching the page and nothing is withheld. A custom thank-you page is unaffected: it resolves the order from the buyer's own session, so its customer data is always complete.
