...
* Fixed: two option descriptions on the settings screen (Page variables → Post data) described the output incorrectly. "Category list of current post/archive" said it sends the category *names*, while `pageCategory` has always contained the category **slugs** (`news-and-events`, not `News and Events`) - so a Google Tag Manager trigger written against the description could never match. "Post date" said it adds 4 data layer variables, while it has added 9 since 1.15: `pagePostDate`, `pagePostDateYear`, `pagePostDateMonth`, `pagePostDateDay`, `pagePostDateDayName`, `pagePostDateHour`, `pagePostDateMinute`, `pagePostDateIso` and `pagePostDateUnix`. Both descriptions now match what is sent; no data layer value changed.
...
* Fixed: the WooCommerce and form-interaction trackers no longer register their event listeners twice if their script is loaded a second time on the same page - by an AJAX navigation, or by a page builder that duplicates the script handle. Every ecommerce and form event was pushed twice in that situation. The media and Contact Form 7 trackers already had this protection; these two did not.
