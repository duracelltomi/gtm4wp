...
* Added: the Google Consent Mode default state can now be **scoped to regions** ("Apply the defaults in", beta): the consent default command gains the standard `region` parameter, so a default written for GDPR no longer denies consent for every visitor worldwide - visitors outside the selected regions are simply not affected by it. Ships with an "EEA, United Kingdom and Switzerland" preset - covering the EU-27 plus Iceland, Liechtenstein, Norway, the United Kingdom and Switzerland - and a custom mode accepting ISO 3166-1/-2 codes (`DK`, `DE-BY`, `US-CA`; invalid entries are dropped on save). A companion **"Wait for consent update"** option (beta) adds the `wait_for_update` parameter, giving your consent management tool the configured number of milliseconds to deliver the visitor's stored choice before tags proceed with the default state - without it, a slowly loading consent tool can lose the race and the visit is measured with the (usually denied) default; 500 ms is the value Google's own plugins use. The whole default payload is filterable through the new `gtm4wp_consent_mode_default_payload` filter, so an integration can set its own region list without a stored option. With none of the new options configured, the emitted consent block is byte-identical to previous versions.
* Added: the two **advanced consent mode** parameters that keep denied-consent traffic partially measurable, each as its own opt-in checkbox (beta): "Redact ads data without ad storage consent" emits `gtag("set", "ads_data_redaction", true)`, removing ad click identifiers from Google's network requests while `ad_storage` is denied, and "Pass ad click information through URLs" emits `gtag("set", "url_passthrough", true)`, carrying `gclid`-style ad click parameters across internal navigation via the URL instead of cookies. Both are emitted after the consent default command, so the default state is always the first consent command on the page.
