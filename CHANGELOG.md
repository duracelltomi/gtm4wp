...
* Changed: the **"Only output the container on production environments"** option (Google Tag Manager container → Advanced) now tells you what it will actually do on *this* site. Whether the option has any effect depends entirely on `WP_ENVIRONMENT_TYPE`, which lives in `wp-config.php` or the server config and is invisible from the admin - so its description now also reports the environment type WordPress currently returns here and whether the container would be loaded or suppressed. It also calls out the common trap: when `WP_ENVIRONMENT_TYPE` is not set at all, WordPress falls back to "production", so the option silently keeps loading the container - the description now says so and names the values to set on your non-production copies. The option's behavior itself is unchanged.
* Fixed: on the cache-safe data layer (issue #398) the two WooCommerce one-shot events - the `add_to_cart` after a cart "Undo" and the reliable-purchase fallback - are now actually delivered. WooCommerce does not start its session on a REST request, so the first-party session endpoint could not read the queued event and, because the mode also keeps these events out of the cached HTML, the `add_to_cart` and the fallback `purchase` were silently dropped. The endpoint now loads the WooCommerce session for the current request (only when a one-shot is actually pending, so an ordinary anonymous visitor still pays no WooCommerce cost), so both events fire as intended.
* Fixed: the cache-safe session endpoint (issue #398) is now genuinely read-only. It is a public, unauthenticated `GET`, so it must not change state - but it was consuming the WooCommerce session markers for the queued one-shot events while resolving them, which meant any cross-site link to the endpoint (a `GET` navigation carries the visitor's cookies) could destroy a real buyer's pending `purchase`/`add_to_cart` event before their own page delivered it. The endpoint now only reads those markers; consuming them and flagging the order tracked happen on the existing authenticated `POST` beacon, which now covers the re-add event too.
* Fixed: the cache-safe data layer (issue #398) no longer stops delivering the visitor IP, Cloudflare country and one-shot events on long-lived cached pages. The REST nonce was baked into the cached page HTML and expires after ~24 hours, after which WordPress rejected the request; the anonymous once-per-session read now sends no nonce (it needs none), and the confirm beacon uses a fresh nonce returned by the endpoint, so both keep working no matter how long the page was cached.
* Fixed: the optional **"Track dynamically inserted players"** setting (Media events) no longer risks hanging the tab on pages with a Spotify embed. Spotify's player API replaces the embed element with its own iframe, which the new-player watcher then saw as a fresh embed and wired again, looping. The watcher now recognises an SDK-replaced element, so each player is wired exactly once (Spotify was the affected provider; the fix covers any provider whose SDK swaps the element).
* Fixed: on sites using **PublishPress Authors**, a post with a single **guest** author now reports that guest as `pagePostAuthor` / `pagePostAuthorID`. Previously only posts with more than one author used PublishPress data; a single-author post always fell back to the WordPress user who created the post, which is the wrong name for a guest author. The `pagePostAuthors` / `pagePostAuthorIDs` arrays still appear only when there really is more than one author. The author name/ID are also now read reliably from PublishPress's author objects regardless of how they expose those fields internally, so they can no longer come through blank.
* Fixed: on the cache-safe data layer (issue #398), the rare response that clears the internal JS-readable login marker for a visitor whose login has lapsed is now marked "do not store", so a full-page cache can never capture that cookie-clear and replay it to other visitors.
* Fixed: the **HTML5 media** tracker no longer fires every percentage-progress milestone at once for a media element that briefly reports a zero duration (e.g. a live stream before metadata loads). It now applies the same zero-duration guard as the other media trackers.
* Fixed: the **Twitch** tracker no longer risks a duplicate element id if its script is injected onto the page more than once (some tag-manager setups do this), which could bind the player controls to the wrong container. The container ids are now unique across re-injection.
* Fixed: the deprecated `gtm4wp_eec_product_array` filter no longer emits its `WP_DEBUG` deprecation notice once per product (so a category page with many products produced one notice each). The notice is now shown once per request; the filter itself keeps running for every product exactly as before.
* Added: a new experimental **"Cache-safe data layer"** setting (issue #398). On sites with full-page caching (LiteSpeed, WP Rocket, Varnish, Cloudflare APO) the HTML built for one visitor is served to everyone, so any visitor-specific value baked into the data layer would leak - the classic case being a logged-in editor's page cached with their email/username/role and then served to anonymous visitors. With this on, the values the browser can compute itself (the search term and the referring page) are pushed client-side as a `gtm4wp.visitorData` data layer event under the **same** variable names (`siteSearchTerm`, `siteSearchFrom`), so your Google Tag Manager tags keep working; moving them off the server also removes their reflected-XSS surface. This is Phase 1: while the mode is on, the remaining server-only visitor/session fields (visitor IP, Cloudflare country, the logged-in-user fields, and the WooCommerce customer-data and cart blocks plus the one-shot cart/purchase events) are **omitted** from the cached HTML rather than leaked - a later release will deliver them client-side, once per session and cookie-gated. **Off by default** (experimental); when off, the data layer is exactly as before.
* Added: **Cache-safe data layer — Phase 3 (final)** (issue #398). The last two WooCommerce events that Phase 1/2 still *omitted* on full-page-cached sites — the `add_to_cart` fired when a product is restored to the cart (the cart "Undo") and the reliable-purchase **fallback** that recovers a `purchase` when the order-received page was missed (a custom/redirect thank-you page) — are now **delivered client-side** too, so with the mode on **no** visitor or session data is rendered into cacheable HTML at all. Because each of these must fire **exactly once**, they are delivered differently from the other visitor data: when one is queued, GTM4WP sets a short-lived event cookie, and only then does the browser fetch it (from the same first-party session endpoint, resolving the order/re-add from *this* session — never from a URL parameter), fire the event once and clear the cookie; an anonymous visitor on a cached page, who never has the cookie, never fetches. The purchase fallback **reuses the existing de-duplication** — the same `gtm4wp_orderid_tracked` browser guard (keyed on the order number) the order-received page writes, plus the server `_ga_tracked` order flag — so a fallback fire and a real order-received purchase for the same order can never both count; the re-add is de-duped on a per-event token. The "Do not flag orders as being tracked" option is fully honoured (with it on, no order-tracked state is written anywhere — server or browser). Both events carry their revenue/values hex-encoded over the wire, so a hostile order number can never break out. This completes the cache-safe data layer; it remains **off by default** (experimental).
* Fixed: the reliable-purchase **fallback** on the cache-safe data layer (issue #398) can no longer double-count a `purchase` across devices. Because the fallback is delivered over a read-only endpoint, it deliberately did not set the server `_ga_tracked` order flag (a GET must not change order state), so it de-duplicated only *within one browser*: if the order-received page was missed on device A (the fallback fired there) and the buyer then opened the real thank-you URL on device B, both could count. Now, after delivering the fallback purchase, the browser fires a single authenticated `POST` beacon (protected by the REST nonce) that flags the order tracked server-side — and the order id is taken only from the buyer's own session, never the request body, so it can never flag another order, and the GET itself stays side-effect-free. A later order-received render on **any** device is then suppressed by the `_ga_tracked` flag, so the purchase counts **exactly once**. The beacon fires only on the rare fallback delivery (never a new per-page request), a failed beacon simply degrades to the previous same-browser behavior, and the "Do not flag orders as being tracked" option is honoured end to end (no marker, no beacon, no flag).
* Added: **Cache-safe data layer — Phase 2** (issue #398). The server-only visitor fields that Phase 1 only *omitted* on full-page-cached sites are now **delivered client-side**, still under the same `gtm4wp.visitorData` variable names, with no unconditional per-page request. The visitor IP, Cloudflare country and logged-in-user data (login state, role, email + hash, registration date, username, id) come from a new first-party session endpoint that returns **only the current request's own data** (it takes no user/session id, so one visitor can never request another's) with no-cache headers: the IP/country are fetched once per session and cached in the browser, the user data only when the login state changed, so an anonymous visitor on a cached page never fetches and never receives user data. The WooCommerce customer and cart blocks ride the cart-fragments response WooCommerce already refreshes on every cart change, so no extra request is added. All of these are gathered into a **single** `gtm4wp.visitorData` data layer push (on a cached page view it is synchronous; on the first view of a session it fires when the endpoint responds), so a GTM setup sees the values arrive together — point any tags that read them at the `gtm4wp.visitorData` Custom Event. All request-header values round-trip safely (hex-encoded), and the mode stays **off by default** (experimental). Still deferred to a later step: the two WooCommerce one-shot events (an `add_to_cart` after the cart "Undo" and the reliable-purchase fallback), which need per-event de-duplication to be delivered safely and so remain omitted while the mode is on.
* Added: an optional **"Track dynamically inserted players"** setting (Media events). The media trackers wire up players that are on the page when it loads; with this on they also track players inserted *after* load - opened in a popup/lightbox or loaded via AJAX - which previously went untracked. It works by watching the page for new embeds with a single shared `MutationObserver` (one for all providers, scanning only the nodes each change adds, not the whole document), then running the exact same wiring a player gets at page load. **Off by default** (experimental): the observer has a small per-DOM-change cost, so enable it only if your site injects media players at runtime. The Wistia tracker already handled runtime insertion natively and is unchanged. Thanks for the request (#3).
* Added: **PublishPress Authors support** for the Page variables author data. On sites that use PublishPress Authors, a post can have several authors (co-authors and guest authors) - important for E-E-A-T. When such a post has more than one author, GTM4WP now also outputs `pagePostAuthors` (the list of author names) and `pagePostAuthorIDs` (the list of author IDs; guest authors use a negative id) next to the existing single-value `pagePostAuthor` / `pagePostAuthorID` variables, which stay for back-compat and now point at the primary (first) author. The two arrays are filterable via `gtm4wp_page_post_authors` and `gtm4wp_page_post_author_ids`. Uses the existing "Post author name" / "Post author ID" options as the on/off switches; when PublishPress Authors is not active, behavior is unchanged.
* Added: an optional **"Include parent categories in the category list"** setting (Page variables → Post data). By default the `pageCategory` data layer variable lists only the categories directly assigned to the current post or archive. With this on, the parent (ancestor) categories of each category are also added - immediate parent first, up to the top-level category - and the list is de-duplicated. **Off by default**, so the current output is unchanged until you enable it. Thanks to @twentyfortysix for the original patch (#220).
* Fixed: the "Include parent categories in the category list" option now correctly greys out in the settings screen while its parent option, "Category list of current post/archive", is turned off - the two could previously be set to the contradictory combination of parent categories on with the category list off (the value was already ignored on the frontend in that case, so tracking was unaffected). Any option can now declare such a dependency, so the control is disabled until the option it relies on is enabled.
* Added: **export & import of the plugin settings** (Google Tag Manager settings screen, next to "Save changes"). "Export settings" downloads all of your GTM4WP options as a JSON file; "Import settings" reads such a file back on another site (or the same one). The import is treated as untrusted: every value in the file is run back through the exact same per-field sanitizers as a normal save before anything is stored - unknown keys are dropped, oversized files are refused and the file is only ever parsed with `json_decode()` (never `unserialize`/`eval`), so a hand-edited or malicious file cannot inject unsafe data. Both endpoints require the settings capability and a nonce. The value set is schema driven, so every current and future option is covered automatically.
* Fixed: importing a hand-edited settings file whose value has the wrong shape (e.g. a list where a single value is expected) no longer emits a PHP "Array to string conversion" warning; every option value is now safely normalized to the field's expected type during import, exactly as on a normal save.
* Added: a kill switch to stop the Google Tag Manager container from loading on a cloned or staging copy of a site without deactivating the plugin. A new **"Only output the container on production environments"** option (Google Tag Manager container → Advanced) emits the container only when WordPress reports the environment type as "production" (set `WP_ENVIRONMENT_TYPE` on your non-production copies; it defaults to "production" when unset). For host-based control from an mu-plugin or `wp-config.php`, the new `gtm4wp_output_container` filter (default `true`) can veto the container from PHP. In both cases the data layer stays active and only the container `<script>` / `<noscript>` is suppressed - exactly like the "Off" placement. **Off by default** (experimental).
* Fixed: WooCommerce Cart-block cross-sell `view_item_list` / `select_item` events no longer carry the plugin's internal `internal_id` bookkeeping key in their GA4 item; it is now stripped for cross-sells exactly as it already was for cart lines and every classic tracking path.
...
* Changed: the `gtm4wp_eec_product_array` filter is now deprecated in favor of the new `gtm4wp_eec_item_with_source` filter (which additionally receives the source cart/order item). It keeps working unchanged - it still receives the same two arguments and still runs *before* the new filter, so both filters can modify the item - and only raises a deprecation notice when `WP_DEBUG` is enabled. Existing integrations need no change.
...
* Fixed: the "Taxonomy to be used for product brands" dropdown again lists only product taxonomies a store owner would use as a brand (public, UI-visible, non-builtin), matching 1.x. It no longer offers WooCommerce's internal `product_type`/`product_visibility` taxonomies or cross-cutting ones other plugins attach to products (e.g. WPML's non-public "Translation Priorities").
...
* Fixed: the Contact Form 7 tracker no longer double-registers its data layer events when the script runs twice (e.g. re-injected by a page builder or after an AJAX navigation).
...
* Updated: Contact Form 7 integration modernized against the current CF7 DOM events. Three new events are tracked - `gtm4wp.contactForm7BeforeSubmit` (before validation), `gtm4wp.contactForm7Unaccepted` (acceptance/terms checkbox not ticked) and `gtm4wp.contactForm7Aborted` (submission aborted) - and every CF7 data layer push now also carries the form's `unittag`, `containerpostid`, `locale` and `status` (the existing `formid` and `inputs` fields are unchanged). File inputs are reported by their file name instead of a raw File object.
