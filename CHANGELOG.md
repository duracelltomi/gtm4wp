...
* Fixed: the GA4 `item_id` now reaches the data layer as a string for every product. A product without a SKU reported its numeric product id as a JSON number while a product with a SKU reported a string, so on a store mixing the two, Merchant Center feeds and any GA4/GTM matching on `item_id` only lined up for part of the catalog - and the same product changed type the day a SKU was added to it. The Google Ads dynamic remarketing field (`id`, `destination`, ...) derives from `item_id` and is consistently a string as well now (it already was whenever a feed id prefix was configured). GTM's own "equals" trigger comparisons keep matching either way; only a Custom JavaScript variable comparing `item_id` with strict equality (`===`) against a number needs updating to the string form. The Easy Digital Downloads integration (new in this version) reports `item_id` the same way. The `sku` field follows the same rule: on a product without a SKU it falls back to that same product id, and that fallback is a string now as well.
