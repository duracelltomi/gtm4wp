...
* Changed: a player's third-party script is now requested only on pages that actually contain one of its embeds. Enabling a player used to load its script on every page of the site, whether or not any page used that player - with every player enabled that was 288 KB fetched from seven different providers on every single page view, and it also handed those providers each visitor's IP address and browser details on pages with no player at all. Each tracker now fetches its own script, and only after it finds a matching embed on the finished page. That check also covers players the page's stored content cannot be searched for: ones placed in a widget, a block template, a page builder or a shortcode's output, and ones opened later in a popup. Tracking behaviour is otherwise unchanged. One consequence is worth knowing if you use a consent plugin that blocks third-party scripts: the request is now made by the tracker rather than written into the page, so there is no script tag naming the provider's own domain for such a plugin to recognise. Two levers replace it. Blocking or dequeuing one player's own script handle (`gtm4wp-vimeo`, `gtm4wp-twitch` and so on) stops that provider's request, because it stops the tracker that would make it. GTM4WP also ships a script whose only job is to be blocked: `gtm4wp-media-gate.js`, a real enqueued script every media tracker depends on. Block or dequeue that one handle and no media provider is contacted at all, while the trackers still run and still report players already on the page - which gives a consent plugin a single, ordinary script tag to recognise. The same decision is available server-side through the new `gtm4wp_media_sdk_blocked` filter.
