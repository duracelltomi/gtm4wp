...
- `src/` — OOP source (PSR-4 `GTM4WP\`)
  - `src/Abilities/` — the WordPress Abilities API surface (WordPress 6.9+): one `Registrar`, one `Meta` builder, `ProviderInterface` and the two plugin-wide providers (`StatusAbilities`, `SettingsAbilities`); a feature's abilities live in its module (`src/Modules/<Name>/Abilities.php`) and reach the `Registrar` through the opt-in `Module\AbilitiesInterface` on the module's `AdminSchema` (the `SiteHealthInfoInterface` pattern; a module's master switch and host plugin come through `Module\StatusInfoInterface`, the Container slice of `get-status` from `Container\StatusReport`; the registry is walked with `instanceof`, so a third-party module contributes the same way - rule PA-21 in `.security/code-review-patterns.md`); every ability is gated on `Capability::can_manage_settings()` and pinned by `tests/unit/Abilities/ContractTest.php`; the plan and ledger live in the git-ignored `.roadmap/abilities-api-plan.md`
