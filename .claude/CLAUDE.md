...
- `src/` — OOP source (PSR-4 `GTM4WP\`)
  - `src/Abilities/` — the WordPress Abilities API surface (WordPress 6.9+): one `Registrar`, one `Meta` builder, `ProviderInterface` and the two plugin-wide providers (`StatusAbilities`, `SettingsAbilities`); a feature's abilities live in its module (`src/Modules/<Name>/Abilities.php`) and reach the `Registrar` through the opt-in `Module\AbilitiesInterface` on the module's `AdminSchema` (the `SiteHealthInfoInterface` pattern; a module's master switch and host plugin come through `Module\StatusInfoInterface`, the Container slice of `get-status` from `Container\StatusReport`; the registry is walked with `instanceof`, so a third-party module contributes the same way - rule PA-21 in `.security/code-review-patterns.md`); every ability is gated on `Capability::can_manage_settings()` (a write additionally on the `gtm4wp_abilities_allow_write` switch, through `Registrar::can_write()`, and is registered only while that switch is on) and pinned by `tests/unit/Abilities/ContractTest.php`; the plan and ledger live in the git-ignored `.roadmap/abilities-api-plan.md`
  - `src/Admin/` — settings page, REST controller, notices, plugin-row links, and the two Site Health collectors: `SiteHealthInfo` (the Info section, rows from every module via `Module\SiteHealthInfoInterface`) and `SiteHealthTests` (the Status tab: the plugin-wide configuration test plus module tests via `Module\SiteHealthTestsInterface`, ids `gtm4wp_<module>_<key>`); rows are built with `SiteHealthRows` so every row carries an English `debug` twin (the copied text prints the KEY and that twin — U159) and the disclosure rule on the interface applies to every row (pasted into public threads)
...
- WooCommerce >= 5.0 (tested up to 11.1.0)
