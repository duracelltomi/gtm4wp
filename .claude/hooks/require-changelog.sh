...
MSG="Production code changed but CHANGELOG.md has no matching entry. Describe this change under the top UNRELEASED heading (the development target named in .claude/RELEASE-STATE.md; if the top heading is an already-released version, open the new heading above it in the same edit), in its theme group where the section has them. IMPORTANT: if this change fixes or extends a feature that was itself introduced in the SAME unreleased version, EDIT that feature's existing bullet instead of adding a new '* Fixed:' bullet (a user upgrading from the last release never ran the intermediate code); editing an existing bullet satisfies this hook. Add a new bullet ('* Added:' / '* Changed:' / '* Updated:' / '* Fixed:') only for a change that is genuinely new versus the last released stable. Pure internal refactors with no functional change need no entry - use [skip changelog] on the commit instead. Mirror user-visible changes into the matching '= version =' block in readme.txt. Full policy: the changelog skill."
