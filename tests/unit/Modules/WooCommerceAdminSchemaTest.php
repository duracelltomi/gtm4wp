...
final class WooCommerceAdminSchemaTest extends TestCase {
	 ...
	 * constant defined. Both legs are written as literals (TS-21): an
	 * expectation computed with the same defined() the source uses cannot go
	 * red in either direction, which is what T98 found.
	 */
	public function test_the_schema_reports_its_master_switch_and_an_absent_host(): void {
		...
		$this->assertFalse( defined( 'WC_VERSION' ), 'Precondition: nothing in the suite defines the host constant in-process (TS-16).' );
	...
	public function test_the_schema_reports_the_installed_host_and_its_version(): void {
		...
		$info = ( new AdminSchema() )->status_info( new Options( ( new WooCommerceModule() )->defaults() ) );
