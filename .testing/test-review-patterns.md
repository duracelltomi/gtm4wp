...
- **The producer of a hand-set precondition is part of the guard (added Run 8,
  2026-08-14).** When every test that needs a request-scoped flag/marker/global sets
  it **by hand in the fixture**, the consumers all discriminate while the code path
  that raises the state in a real request is asserted by nothing — it can be deleted
  with the suite green. Measured on the `6d9e7a7` double-purchase fix: the
  PurchaseTracking half of `gtm4wp_woocommerce_purchase_data_pushed` is fully pinned
  (bail on hand-set flag, intra-class raise-then-bail chain, negative branch), yet
  both PageDataLayer legs — the raise in the order-received render and the fallback's
  honor of it — reverted green individually at 2002/4686 (T51). This is the TS-12
  attachment form generalized from callbacks to state: ask of every hand-set global
  in a fixture, *which test executes the line that sets this in production?*
- **A guard promoted to a shared helper is only revert-provable at call sites whose
  tests feed the failure class (added Run 7, 2026-08-13).** When a fix centralizes a
  guard (`ScriptTag::json_literal()`, 10 callers), the helper's own test pins the
  *mechanism* but not the *routing*: reverting a call site to the unguarded form goes
  red only if some test drives THAT site with the failing input. Run 7 measured it —
  the helper test plus six probe-verified sites left two filter-fed call sites (the
  ones third-party code can actually reach) revertible with the suite green. When a
  guard's fix note says "N of M sites probe-verified", the unlisted M−N are the review
...
**A gate has two halves: the callback and its attachment (added Run 7, 2026-08-13).**
Grepping for `permission_callback` sites and testing each *callback* grant+deny proves
only half the gate. The **registration** — the `register_rest_route()` call that binds
the callback to the route, plus any option gate and the `rest_api_init` wiring around
it — is its own surface: with the callback exemplary-tested (~20 assertions on
`check_confirm_purchase_permission`), swapping both routes' `permission_callback` to
`__return_true` left **all 1965 tests green**, because no test executes
`register_confirm_purchase_route()` at all. This is the #143 lesson ("unit tests call
handlers directly, bypassing `permission_callback`") one hop up. Recipe: capture
`register_rest_route` args and assert the `permission_callback` identity, methods, and
any option gate both ways; pair it with a TC-4 `has_action( 'rest_api_init', … )` test.
When running the A.4 sweep, ask of every site: *who asserts this line, not just this
method?*
...
Two additions from Run 10 (2026-09-03, google-auth panel):

6. **`mockClear()`, never `mockReset()`, on a stand-in that carries a default
   implementation.** The `apiFetch` stand-in's whole value is its documented
   "unconfigured call rejects loudly" default; `mockReset()` in a `beforeEach`
   erases that implementation for the rest of the file, silently downgrading every
   unexpected call to a resolved `undefined` — which a component's own try/catch
   then converts to a quiet error notice instead of a loud failure. `mockClear()`
   clears calls and keeps the contract. **Corollary (Run 11): configure expected
   calls with `mockResolvedValueOnce`/`mockRejectedValueOnce`, never the persistent
   forms** — a persistent `mockResolvedValue` replaces the default implementation
   for the rest of the file just as thoroughly as `mockReset()` would, and
   `mockClear()` does not restore it; the `…Once` value is consumed by the expected
   call and the loud-reject default survives for any unexpected extra request.
7. **Rule 3 has a measured limit: jsdom never delivers a click to a `disabled`
   button, so a handler-level re-check behind a rendered `disabled` prop cannot be
   discriminated through the UI.** Measured on the panel's busy guard: with the
   handler re-checks deleted the deferred-promise double-submit test stayed green,
   and it went red only when `disabled` was neutralized too — so the test pins the
   *prop* layer, and the handler layer (the part rule 3 exists to demand, because
   prop pass-through is the untrusted half in production) is an in-harness blind
   spot of the TS-17 class. Write the double-submit test anyway (it guards the
   prop layer), add the handler re-check anyway (TableControl precedent), and
   record the blind spot rather than letting the green test imply the handler is
   pinned.
...
| 2026-08-13 (Run 7 — report only) | Extended **TS-12** with the **attachment form** (a gate has two halves — the callback and its registration; the exemplary-tested confirm-purchase callback's routes swap to `__return_true` with 1965 tests green because no test executes `register_confirm_purchase_route()`) and **TS-15** with the **shared-helper corollary** (a promoted guard is only revert-provable at call sites whose tests feed the failure class — `json_literal`'s two filter-fed callers revert green while the helper test stays green; the unlisted sites in an "N of M probe-verified" fix note are the review lead). Both measured by this run's six serialized revert probes, all six confirming (T39–T44). Third confirmation of **TS-13/TS-17's** "the test supplies the collaborator's only behavior": every consent-gate case hand-sets `window.gtm4wp_media_sdk_allowed`, so emptying `gtm4wp-media-gate.js` — the file that creates it — leaves all 713 JS tests green (T41). Positive counter-examples worth copying, no numbered entry: the Dailymotion fake `player.on()` **throws on any event name outside the real vocabulary** (a stand-in stricter than the SDK — TS-13 inverted deliberately); the phone corpus derives `expected` from upstream metadata, never from the code under test (the anti-TS-4 oracle shape); and the HU phone case is labeled in-file as "green against pre-fix code — must-not-break, not a regression test" (anti-TS-14 honesty: stating which assertions discriminate and which merely hold). No new numbered pattern — both finds generalize existing entries. |
