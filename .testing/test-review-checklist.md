...
|---|---|---|---|---|---|---|---|
| **Plugin Bootstrap** (main file, `uninstall.php`, `Plugin.php`, `Autoloader.php`, **`src/RestCors.php`**, **`src/RequestOrigin.php`** — the shared guest-route gate, extracted in phase 3 (`9c94bb8`, ledger-only), documented-indirect via `PageDataLayerTest` (20 permission cases, real class) + `GoogleDataManagerBackfillTest`; **`Plugin::boot_refund_lane()` (phase 4)**; **`src/Capability.php`** (2026-09-21 — the one spelling of the settings-capability filter, `CapabilityTest` 6 cases incl. input-argument tolerance; the abilities wiring in `boot()` is tracked in the Abilities layer row)) | `RestCorsTest` (5, incl. wrapper + non-request guard + priority-11 + namespace consistency); `Admin/AdminCapabilityFilterTest` (the `Plugin::boot()` gate, T30; + KeyNotice attachment, T74); **`PluginRestWiringTest`** (the `rest_api_init` closure — settings + google routes + RestCors, T66, probe-red; + the GDM destinations route incl. callback identity AND the service-account deletion veto attach+effect from a real `boot()`, Run 11; + backfill route both directions of its option gate, privacy boot attachment, relabel route identity, Run 12); `UninstallTest` (#240 `as_unschedule_all_actions` tuple pinned with the library defaults — revert red; subprocess guard halt); rest BE-3 N/A | [x] 2026-09-20 | [x] 2026-09-20 | [x] 2026-08-05 | [x] 2026-08-05 | [x] 2026-09-20 (#240 pinned; T82 closed 2026-09-21 — `boot_refund_lane()` asserted from a real `boot()` both ways, probe-red; T94a replay-gate options attachment probe-red; Run 13: `Registrar::register_hooks()` attachment from a real `boot()` probe-red C5 via **`PluginAbilitiesWiringTest`**) | [x] 2026-09-22 |
| **Abilities layer** (`src/Abilities/` — **5 files**: `Registrar` 256, `Meta` 87, `ProviderInterface` 28 (BE-3 N/A), `StatusAbilities` 370, `SettingsAbilities` 586; plus the shared services the abilities and the REST routes both call: `src/Admin/SettingsStore.php` 395, `src/Admin/ConfigurationChecks.php` 290, `src/Modules/Container/StatusReport.php` 116, `src/Modules/GoogleDataManager/{DestinationProbe,RefundReplay}.php` 76/101, and the per-module providers `src/Modules/{GoogleAuth,GoogleDataManager}/Abilities.php` 205/375; 11 abilities, WordPress 6.9+; landed 2026-09-21/22 `ccb985b..30f620c`, **row added Run 13** — the branch had no row, the absent-not-`[ ]` failure a third time) | `Abilities/ContractTest` (12 / 869 — the catalogue pinned exactly, gate identity per ability, recursive schema types, annotations, `confirm`, translatable descriptions), `RegistrarTest` (17: both core actions, off switch, write switch both halves, third-party opt-in / no-interface / missing-class walk), `SettingsAbilitiesTest` (39), `StatusAbilitiesTest` (13), `MetaTest` (5), `AbilitiesTestCase` (real registry + `OptionStoreTrait`), third-party fixtures ×6; `Admin/SettingsStoreTest` (14), `Admin/ConfigurationChecksTest` (13), `Modules/ContainerStatusReportTest` (11), `Modules/GoogleAuthAbilitiesTest` (15), `Modules/GoogleDataManagerAbilitiesTest` (28), `PluginAbilitiesWiringTest` (3, real `boot()`), `CapabilityTest` (6) | [x] 2026-09-22 | [x] 2026-09-22 (**T101** get-settings under a hardcoded constant unpinned at the ability level, probe-green P7; **T104** replay/log/destination registration constraints unpinned, P2; T105a/d/e/g residue) | [x] 2026-09-22 (**T99** the `private` Site Health row exclusion revertible green, P6; **T102** `StatusReport` row booleans + `hardcoded.errors`, P5a/b/c; T105b/c/f) | [x] 2026-09-22 (raw passthrough with the escaper stubbed; hostile update + import both directions through the real `Field` sanitizers; disclosure with secret-bearing fixtures — C2 probe-red; #259 cap ×3 — C4 probe-red) | [x] 2026-09-22 (#251 recursive type walk, #254 `locks_any` agreement, #256 source grep, #259 ×3 — each red-before per the R34 fix session, #259 re-probed red this run) | [x] 2026-09-22 (`AbilitiesTestCase` stubs everything the real registry reaches; every file green alone by path + seeds) |
...
| **Container Module** (`src/Modules/Container/` — incl. **`HardcodedContainers`** (`locks_any()` #254), **`StatusReport`** (2026-09-21, the get-status container slice; `PLACEMENTS` is the one definition the schema enum reads)) | `Modules/ContainerRowsTest`, `ContainerAdminSchemaTest`, **`HardcodedContainersTest`**, **`ContainerStatusReportTest`** (11, incl. the separate-process `locks_any` agreement) | [x] 2026-09-22 | [x] 2026-09-22 | [x] 2026-09-22 (#223 `/D` anchors pinned ×4 both directions + wp-config subprocess leg; **T102 closed 2026-09-22** — `StatusReport` row booleans `environment`/`omit_id`, `hardcoded.errors` and the `placement_name()` default leg probe-green, Run 13) | [x] 2026-09-02 | [x] 2026-09-02 | [x] 2026-09-02 |
...
| **EasyDigitalDownloads Module** (`src/Modules/EasyDigitalDownloads/` — **5 files**: Module 178, DownloadData 919, PageDataLayer 644, ListTracking 217, AdminSchema 389 (+ `status_info()` 2026-09-21); landed 2026-09-01, **row added Run 9**; JS bundle tracked in Frontend JS row) | `Modules/DownloadDataTest`, `EddPageDataLayerTest`, `EddListTrackingTest`, `EasyDigitalDownloadsModuleTest`, **`EasyDigitalDownloadsAdminSchemaTest`** (2, Run 13 — status only), `edd-stubs.php` (models `__get`-without-`__isset` — **the docblock's rationale is wrong** (Run 12, verified from EDD 3.7.0 source): `EDD\Database\Base::__isset()` exists and is true for every declared row property, so the stub is *less* capable than the real row — the safe direction, but RI-12 is a PublishPress fact, not an EDD one; T93f → `/upstream-review` row + docblock fix); AdminSchema swept automatically by the registry-driven `ModuleConsistencyTest` | [x] 2026-09-02 | [x] 2026-09-22 (**T98/T103 closed 2026-09-22** — `status_info()` host-present leg mirrors the source expression (TS-21, probe-green alone, order-sticky red in-suite); `fields()` has none of the WC sibling's pins, probe-green P10; Run 13) | [x] 2026-09-20 (T58/T59/T62 closed; T96a closed 2026-09-21 — EDD decode pinned at the item and at the hex sink) | [x] 2026-09-02 (T52/T57 closed, both probe-red) | [x] 2026-09-02 (#217/#219/#220 exemplary — grant+deny+withholding, red pre-fix; #218 discriminating double-init; #222 guard now pinned, T54) | [x] 2026-09-02 |
...
| **ContactForm7 Module** (`src/Modules/ContactForm7/` — incl. tracker JS; GA4 `form_status` + submit-before-lead ordering, 2026-08; master-language title override, 2026-09; `status_info()` 2026-09-21) | `Modules/ContactForm7ModuleTest` (master-language on + off-gate with different-master-value fixtures), `ModuleHooksTest` (gate); JS: `contact-form-7-tracker` (order-as-effect test) | [x] 2026-09-02 | [x] 2026-09-22 (**T98 closed 2026-09-22** — `status_info()` host-present leg mirrors the source expression, probe-green P3c; Run 13) | [x] 2026-09-02 (T63's optional master-path TS-11 leg is Low) | [x] 2026-09-02 | [x] 2026-09-02 | [x] 2026-09-02 |
...
| **Admin — Notices/AJAX** (`src/Admin/Notices.php` — renders from the shared **`ConfigurationChecks`** since 2026-09-21, tracked in the Abilities layer row) | `Admin/NoticesTest` (#110 encoder-discriminating nonce guard; #131 core-faithful `sanitize_key` pair; invalid-dataLayer-name notice grant+deny; Run 13: the same 30 tests through the real `ConfigurationChecks`, nothing lost in the extraction) | [x] 2026-09-22 | [x] 2026-09-22 (**T100 closed 2026-09-22** — `print_notice()` rendering contract: the `?` in `data-href` the inline dismiss JS strips, `is-dismissible`, severity class, link-absent leg — probe-green P4a/P4b) | [x] 2026-08-13 | [x] 2026-08-13 (T105h: message escaping stubbed identity, Low, pre-existing) | [x] 2026-09-22 (C3 filtered-cap hardcode probe-red) | [x] 2026-08-13 |
| **Admin — Settings UI** (`src/Admin/SettingsPage.php` — boot-fallback notice `232fea6`, `Admin.php`, `RestController.php`, `PluginRow.php`, **`Docs.php` (2026-08-11)**, **`SiteHealthInfo.php`** (2026-09-19 `7890b81`, the plugin-wide Site Health Info section — core `esc_html`s every value, so the collector must NOT pre-escape); **`SettingsStore.php`** + **`ConfigurationChecks.php`** (2026-09-21 phase-1 extractions — the store is the one write path of the REST route, the settings screen and the abilities; `RestController` is route handlers only since #255)) | `Admin/RestControllerTest`, **`SettingsStoreTest`** (14 — `values_hash()` stable/changes, the three `current_values` cases moved in), **`ConfigurationChecksTest`** (13), `SettingsPageTest` (+ columnChoices merge both directions, Run 11), `PluginRowTest`, `AdminCapabilityFilterTest` (+ KeyNotice AND HealthNotice boot attachments — T74/T75, both probe-red), **`DocsTest`** (209 lines: allow-list `->with`, #152/#158 both-directions) + fixtures `UndocumentedThirdParty{Module,Schema}`, **`SiteHealthInfoTest`** (10: instanceof opt-in without fatal, non-array row dropped, junk filter ignored, prefixed keys as written, no empty heading; own stubs, green alone) + fixtures `ReportingThirdParty{Module,Schema}`, `MalformedReportingSchema` | [x] 2026-09-22 | [x] 2026-09-22 (Run 13: nothing lost in the `RestController` → `SettingsStore` move, every pre-move pin re-pinned through the store; T105b/e residue Low) | [x] 2026-09-20 (T60 closed — byte-exact containment, probe-red) | [x] 2026-09-20 (T95a closed 2026-09-21 — raw `& " < '` row through the collector with the real escaper stubbed, probe-red) | [x] 2026-09-21 (T75 closed 09-03; **T83 closed** — the third recurrence of the attachment lesson in `Admin::boot()`, both Site Health wirings now asserted grant + deny, probe-red) | [x] 2026-09-20 |
| **Module Admin Schemas** (`src/Modules/*/AdminSchema.php` — **14 files** (counting rule `ls src/Modules/*/AdminSchema.php`, Run 13); custom sanitizers; doc-link additions across all schemas 2026-08; **`StatusInfoInterface::status_info()`** on WC/EDD/CF7 and **`AbilitiesInterface::abilities()`** on GoogleAuth/GDM, 2026-09-21/22) | `Modules/ModuleConsistencyTest` (non-scalar sweep + doc sweep), `ContainerAdminSchemaTest`, `WooCommerceAdminSchemaTest`, `PageVariablesAdminSchemaTest`, **`MediaEventsAdminSchemaTest`**, **`BlacklistAdminSchemaTest`** (sanitizers behaviorally tested since T45) | [x] 2026-09-22 | [x] 2026-09-22 (**T98** — the three `status_info()` host-present legs mirror the source expression, probe-green ×3 (TS-21); **T103** — EDD `fields()` has none of the WC sibling's pins, probe-green P10) | [x] 2026-08-13 | [x] 2026-08-13 (T45 closed same session) | [-] | [x] 2026-08-13 |
...
|---|---|---|
| **Test-order independence** (`vendor/bin/phpunit --order-by=random` — TS-8/TS-16) | 2026-09-22 (Run 13 + closes) | ✅ **Clean after the closes — declaration + 3 seeds green at 3114/13316, identical counts** (at report stage 3075/9675, seeds 1790095989 / 1790096010 / 1790096034) (seeds 1790095989 / 1790096010 / 1790096034); every new ability/service file green alone by path and under seeds (deep-reads). No hazard in range: `AbilitiesTestCase` stubs everything the real registry reaches, in the base class of each file. One order-sticky constant noted, not a failure: `EDD_VERSION` defined by `EasyDigitalDownloadsModuleTest` makes the EDD `status_info()` mutation red in-suite and green alone (T98). |
| ~~Test-order independence (Run 12 + closes)~~ | 2026-09-21 (Run 12 + closes) | ✅ **Clean after the closes — declaration + 4 seeds green at 2893/7956, identical counts; T84/T85 fixed and re-measured by path / alone + seeds 4/5.** At report stage: ⚠️ Whole-suite clean — declaration + 3 seeds green at 2852/7809 — but two FILE-LEVEL dependencies the shuffle cannot see (T84, T85, both measured):** `GoogleDataManagerBackfillTest` never requires `edd-stubs.php` and errors 4× when run by path (PHPUnit loads every file before running any, so every seed is green); a `CaptureHooksTest` case depends on an earlier test in the same file defining `edd_update_order_meta` (fails alone; fails under seeds 4/5). New TS-16 corollary: run each changed test file **by path, alone**, in declaration order and under a few seeds. |
...
| **Missing test file** (`src/**/*.php` with no dedicated or documented-indirect test) | 2026-09-22 (Run 13) | **Clean — no real zero-test class.** 63 raw hits over **119** files (was 56/104). The 7 new hits classified: `ProviderInterface`, `AbilitiesInterface`, `StatusInfoInterface` → BE-3 N/A (exercised through the third-party fixtures); `GoogleAuth/Abilities`, `GoogleDataManager/Abilities`, `Container/StatusReport` → tested under flat names in `tests/unit/Modules/` (naming-only misses); `RefundReplay` → indirect via the REST + ability tests, every branch reached → `[-]`; `DestinationProbe` got its own `DestinationProbeTest` at the close (T105f). |
| ~~Missing test file (Run 12)~~ | 2026-09-20 (Run 12) | **Clean — no real zero-test class.** 56 raw hits over **104** files (was 36/82). The 20 new hits classify: 17 GDM basename over-reports (dedicated `GoogleDataManager*Test` suites; `RefundData`/`RefundSource` via `RefundEventTest` + the adapter suites), `SiteHealthInfoInterface` BE-3 N/A (exercised via `SiteHealthInfoTest`'s instanceof opt-in), `RequestOrigin` documented-indirect (real class under 20 `PageDataLayerTest` permission cases + `BackfillTest` both directions), `ConsentRegions` → `Google/ConsentRegionsTest`. |
...
| **Untested public methods** (in classes that *do* have a test file — TS-10) | 2026-09-22 (Run 13) | 11 grep misses across the new classes; 9 reached through the registered execute callbacks or the abilities (`update_settings`, `get_site_health`, `get_service_accounts`, `test_service_account`, `get_log`, `replay_refunds`, `write_disabled_error`, `confirmation_required_error`, `no_input_schema`); `SettingsStore::value_schema()` pinned via the schema tests; `StatusReport::placement_name()` default leg unpinned (T102). |
| ~~Untested public methods (Run 12)~~ | 2026-09-20 (Run 12) | 16 grep misses across the new classes; 13 reached indirectly (`parse_*` via the route/cookie tests, `replay_*`/`is_replayable`/`latest_is_accepted` via the replay suite, `is_same_origin_request`/`has_rest_nonce` via both routes, `clear_failures`, `validate_destination`, `script_config`). **Real, Low (T94b/c):** `SendQueue::is_active()` on both adapters, `RefundSender::register_hooks()`'s inactive-source skip + enqueue-closure platform binding, `AdminSchema::consent_policy_choices()`/`type_choices()`. Coverage corroborates: SendQueue 93.94% (cron else-leg), EddRefunds 93.18% / WooCommerceRefunds 91.86% (T91/T94h legs). |
...
| **Security-input coverage** (every `.security` PA-3/RI-2 sink has a hostile-input test — TS-1/TC-5) | 2026-09-22 (Run 13) | ✅ **Clean on every sink the range adds.** `get-settings` raw passthrough with the escaper stubbed (TS-11 shape); hostile update value sanitized both directions through the real `Field` sanitizers; hostile import envelope re-sanitised value by value; `get-service-accounts` disclosure with a secret-bearing fixture (`client_email` leak probe-red C2); #259 third-party message cap+strip ×3 (probe-red C4); `get-site-health` absence of email/key/property id/raw Google wording. Residue Low: `describe()` strip-tags pinned with a tag-free description (T105a); `NoticesTest` message escaping identity-stubbed (T105h, A4, pre-existing). |
| ~~Security-input coverage (Run 12)~~ | 2026-09-20 (Run 12) | ✅ **Clean on every sink the range adds.** Cookie parser + backfill body as a hostile boundary (`CaptureTest:206-361`, `BackfillTest:838-889`); the one new inline `<script>` sink through the real enqueue with a hostile order key (`GoogleDataManagerModuleTest:361-386` — TAG/AMP/APOS discriminated, QUOT belt-and-braces T93b); `record_status()` both directions (`SendLogTest:361`); hostile Google error text + status names (`RestControllerTest:450`, `IngestSendTest:430`); Site Health section carries no secret; SendLogList is a structured React sink (TC-11). Term-name decode → hex sink both directions with a TC-2-computed expectation (`PageDataLayerTest:2284`). Residue: `SiteHealthInfo` must-not-pre-escape pinned by `'on'` only (T95a); `last_error_class` no hostile case (T94e); `rest_url` sink benign-only, site-config `[-]` (T96b). |
...
| **Regression-per-bug** (every `.security` Known Finding has a live regression test) | 2026-09-22 (Run 13) | ✅ **Clean.** Walked #251–#260: **#251** `ContractTest` recursive type walk (red before, R34 fix session); **#254** `HardcodedContainersTest::test_locks_any_…` + the separate-process agreement case; **#256** provider source grep; **#259** `TokenServiceTest` + `GoogleAuthAbilitiesTest` + `GoogleDataManagerRestControllerTest` — **re-probed red ×3 this run (C4)**; #253/#255/#257 refactors n/a; #252/#258/#260 documentation. |
| ~~Regression-per-bug (Run 12)~~ | 2026-09-20 (Run 12) | ✅ **Clean.** Walked #235–#250: **#236** `WooCommerceModuleTest:237` (red on the old shape); **#240** `UninstallTest:190,217` (whole tuple with the library defaults — revert red); **#242** `WooRefundsTest:470` (+ EDD `:305`); **#243** `BackfillTest:722-767` (object identity / strict int on the backfill path — the creation path is T92); **#244** `SendLogTest:361` (both directions); **#249** `BackfillTest:622-673` (both platforms; revert traces `BackfillEndpoint:234`/`:304`); **#250** `ReceiptPageTest:257-284` (hash printed AND key absent) + hash-refusal guards. #235/#237 process; #238/#239 open design limitations, no guard owed; #245 documented non-issue; #246–#248 ledger. |
...
| **Access-control coverage** (every `permission_callback` / capability gate has a grant+deny test; every filterable capability — `gtm4wp_admin_page_capability` — has a test that the filter customizes the required cap while the default stays unchanged — TS-12/TC-13) | 2026-09-22 (Run 13) | **33 sites / 10 gates** (was 22/8): +11 ability `permission_callback`s on two gates — `Capability::can_manage_settings` (the six inline `apply_filters` sites collapsed into one class; **each site still pins the filter per site** with `->once()->with()`, so a site that hardcodes `manage_options` fails its own tests — probe-red C3 at `Notices::dismiss_notice`) and `Registrar::can_write` (switch AND capability, switch-off never asks `current_user_can`, input-argument tolerance). **All three rungs pinned by the branch's author:** (a) callback grant/deny `CapabilityTest` + `RegistrarTest`; (b) identity per ability `ContractTest::test_every_ability_is_gated_on_the_settings_capability`; (c) attachment from a real `Plugin::boot()` `PluginAbilitiesWiringTest` — **probe-red C5** (delete `register_hooks()`: 1E + 2F). Registration-time withholding of every write + execute-time 403 after the switch flips pinned for all five writes. Nothing owed (T105j: the `Registrar` docblock credits `ContractTest` for the execute-time pins that live in `RegistrarTest` + the provider tests). |
| ~~Access-control coverage (Run 12)~~ | 2026-09-20 (Run 12) | **22 sites / 8 gates** (was 19/7). New gate: `BackfillEndpoint::check_permission` (**A0** guest route) — **all three rungs pinned by its author**: grant/deny/neighbour-host/no-evidence/Referer/nonce (`BackfillTest:239-282`), registration identity + enum + required args (`:200-235`), attachment from a real `Plugin::boot()` both directions of the option gate (`PluginRestWiringTest:253-272`; a `__return_true` swap fails there). New sites on the existing GDM `can_manage`: `send-log` GET + `send-log/replay` POST — rung 1 `RestControllerTest:143-161`, rung 2 identity for all three routes `:163-229`, rung 3 executed from `boot()` but filtered to the first route (Low symmetry pin, T94g). GoogleAuth relabel POST/PUT/PATCH: `can_manage` on every endpoint (`GoogleAuthRestControllerTest:160-204`, `PluginRestWiringTest:178-185`). Nothing owed at any rung. |
...
| **Assertion quality** (coverage driver + mutation testing) | 2026-09-22 (Run 13) | **No coverage driver loaded this run** (PCOV absent) — sweeps + judgment + **17 valid main-thread mutation probes: 11 survived (T97–T105), 5 expected-red controls red, 1 predicted gap already pinned (P11).** The three deep-reads' revert-green predictions held 11/12. **After the closes every survivor is red (P1, P2, P3a–e, P4a–c, P5a–c, P6–P10, P12, P13), and `AbilitiesTestCase::execute()` now validates every ability result against its output schema, so a schema drift fails the provider's own tests.** New smell **TS-21** (expected value computed by the source's own expression) found at three sites. Probe mechanics: `--colors=never`, `.` for a dollar in perl match patterns, `php -l` the mutant first. |
| ~~Assertion quality (Run 12)~~ | 2026-09-20 (Run 12) | **Coverage 96.40% lines (8624/8946), methods 81.58%, classes 49.48%** at report stage (was 96.38%). New GDM classes 93–100% lines; the sub-100% ones name exactly the T91/T94 legs. **Infection still non-functional on this PHP 8.4.15 ZTS Windows build** (unchanged since Run 9). Judgment-layer TS-1 finds this run: the mixed-outcome retry rule with single-account fixtures only (T89), the consent gate with granted-only fixtures (T81). |
...
| T83 | Med | addressed | TS-15 attachment — **T75 recurring a third time in the same `Admin::boot()`**: the `SiteHealth` + `SiteHealthInfo` `register_hooks()` lines (`Admin.php:52-53`) are asserted by nothing (both suites drive hand-built instances). **Closed 2026-09-21** — both `has_filter` assertions in the grant case, both `assertFalse` in the deny case; **probe red** (the two `Admin.php` lines removed). 2026-09-23: the Data Manager `SiteHealth` line became the plugin-wide `Admin\SiteHealthTests` collector; the grant assertion follows it. | `tests/unit/Admin/AdminCapabilityFilterTest.php` |
...
### Run 13: 2026-09-22 abilities-branch pass (`b085e0e..9541679`, 25 commits) — 2026-09-22

Report-only pass; full detail in the git-ignored
`test-review-report-2026-09-22-1907.md`. **Reviewed at: `9541679`.** PHP 3075/9675
(declaration + 3 seeds identical), JS 40/974 unchanged, no coverage driver. Eight
comment-compaction commits verified code-identical (`php -w`). Three parallel
read-only deep-reads; **17 valid mutation probes in the main thread — 11 survived,
5 controls red, 1 predicted gap already pinned.** No latent production bug; three
test docblocks claim host-plugin constants "may be present" that nothing in the
suite defines. Pattern added: **TS-21** (expected value computed by the source's
own expression); TS-15 gains a probe-mechanics note. The Abilities layer had no
matrix row (third occurrence of the absent-not-`[ ]` failure) — added.

**Closed the same session (T97–T105)** on the maintainer's "fix straightforward, ask with options otherwise" go-ahead (four forks answered up front: output-schema walker yes, direct `DestinationProbeTest` yes, the `Registrar` docblock fixed under `[skip changelog]`, one commit). PHP 3075/9675 → 3114/13316, declaration + 3 seeds identical; `phpcs` exit 0 over `tests/unit`; no JS change. Every survivor mutation of the report re-probed **red** against the new tests (P1, P2, P3a–e, P4a–c, P5a–c, P6, P7, P8, P9, P10, P12, P13). One lesson while closing: the first depth-cap payload was sized from the constant it had to exceed and stayed green under a raised cap — TS-21's shape inside a fixture, now a bullet of TS-21.

| # | Sev | Status | Summary | File(s) |
|---|-----|--------|---------|---------|
| T97 | Med | addressed | TS-15/TS-5: `TokenService::test_account()` mints with `$fresh = true` (the "a revoked key surfaces now" promise of the ability and the pre-move REST handler); every adapter test stubs the transient absent, so a cache hit is impossible — `true` → `false` green on all three files (P1). Pin: cached token present, `test_account()` issues a second token request and writes the vault row. **Closed 2026-09-22** — cached token, then `test_account()` sends a second exchange and writes the row; **probe red** (P1). | `tests/unit/Google/TokenServiceTest.php`, `tests/unit/Modules/GoogleAuthAbilitiesTest.php` |
| T98 | Med | addressed | **TS-21** + TS-14 + TS-16: the WC/CF7/EDD `status_info()` tests assert `assertSame( defined('X_VERSION'), $info['integration']['active'] )` and the same ternary for `version` — the expected value is the source's own expression, so `active` → `false` / `version` → `null` are green (P3a/b/c; EDD green alone, red in-suite only because one module test defines `EDD_VERSION` order-stickily). Docblocks claim the constants may be present under random order; `WC_VERSION`/`WPCF7_VERSION` are defined nowhere in `tests/`. Pin: a separate-process case per schema defining the constant and asserting the literal pair, and a literal absent pair; correct the docblocks. **Closed 2026-09-22** — literal absent leg in-process behind a `defined()` precondition (WC/CF7; EDD in its own process) + literal present leg in a separate process per schema, and the ability's rows with all three constants defined; docblocks corrected; **probe red** ×5 (P3a–e). | `tests/unit/Modules/WooCommerceAdminSchemaTest.php`, `ContactForm7ModuleTest.php:267`, `EasyDigitalDownloadsAdminSchemaTest.php:34` |
| T99 | Med | addressed | TS-15/TS-1: the `private` Site Health row exclusion in `get-site-health` (`StatusAbilities:271`) runs in no test — no built-in row and no fixture sets `private`, deleting the skip is green (P6). Pin: a third-party fixture implementing `SiteHealthInfoInterface` with one private + one public row; marker absent, public row present. **Closed 2026-09-22** — `StatusReportingThirdPartySchema` now reports a private and a public Site Health row; value and label of the private one absent from the serialised answer, the public one present under its prefixed key; **probe red** (P6). | `tests/unit/Abilities/StatusAbilitiesTest.php`, `StatusReportingThirdPartySchema.php` |
| T100 | Med | addressed | TS-10/TS-19-style: `Notices::print_notice()`'s generic mapping is unpinned — the `?` in `data-href` the inline dismiss JS strips (`substring(1)`; dropping it is green, P4a, and the dismissal would post a truncated code the allow-list rejects), `is-dismissible` (P4b green), the severity class, the link-absent leg for `option_key === ''`. Pin: one fixture per shape asserting the attribute byte-exact and each class only where it belongs. **Closed 2026-09-22** — three shapes byte-exact (dismissible error, non-dismissible error, warning without a setting link) plus the script's `substring( 1 )` on the other side of the contract; **probe red** ×3 (P4a/b/c). | `tests/unit/Admin/NoticesTest.php` |
| T101 | Med | addressed | TS-1/TS-5 + TS-14: `get-settings` never runs under a hardcoded constant at the ability level — `ui_values()` → raw `current_values()` is green in the file (P7); `locked.locked_rows === true` / `locked_columns` unpinned; the export test named "…not the wp_config override" defines no constant. Pin: extend the existing separate-process case to `GET_SETTINGS` + `EXPORT_SETTINGS`, add an env-auth sibling for `locked_columns`, fix the name. **Closed 2026-09-22** — the row-lock case also runs `GET_SETTINGS` (hardcoded id, `locked_rows`) and `EXPORT_SETTINGS` (stored id); a column-lock case (`gtm_auth` locked and applied, export keeps the stored cell); the export test renamed to what it proves; **probe red** (P7). | `tests/unit/Abilities/SettingsAbilitiesTest.php` |
| T102 | Low | addressed | TS-5/TS-15: `StatusReport::report()` only pins the two extreme rows — `environment` `&&` → OR green (P5a; an auth-only row is what `ConfigurationChecks::incomplete_environments` flags, so the two readers would disagree), the `omit_id` `'0'` clause green (P5b), `hardcoded.errors` → `array()` green (P5c), `placement_name()` default leg unreached. **Closed 2026-09-22** — one-token rows, `'0'` no_id, empty table, placement 99 → footer, a malformed `ENV_PREVIEW` constant → `errors` in a separate process; **probe red** ×3 (P5a/b/c). | `tests/unit/Modules/ContainerStatusReportTest.php` |
| T103 | Low | addressed | TS-18: EDD `AdminSchema::fields()` has none of the WC sibling's pins — dropping the `_builtin` clause from the brand-taxonomy filter is green (P10) where `WooCommerceAdminSchemaTest:82` goes red; brand sanitizer + payment-status choices/fallback unpinned; the new test file's docblock credits `ModuleConsistencyTest`/the module test, which pin neither. Run 12's "over-report" classification of the missing file is withdrawn. **Closed 2026-09-22** — taxonomy filter, degrade, sanitizer, fallback statuses (in-process, `function_exists` precondition) and registry statuses (separate process, since a stub would stay defined for every later file); **probe red** (P10). | `tests/unit/Modules/EasyDigitalDownloadsAdminSchemaTest.php` |
| T104 | Low | addressed | TS-18/TS-10: the ability side of the replay input schema (`references.items` type, P2 green), `required => ['measurement_id']` on the destination test and the log ability's `limit` bounds/defaults are pinned by nothing (the REST `args` are, T94g; `ContractTest::assert_typed` only checks a `type` key exists). Sink safe (callbacks re-validate); contract not. Pin: `assertSame` the whole `input_schema['properties']` per write, as the GoogleAuth file does. **Closed 2026-09-22** — three schema pins (log bounds and defaults, `required` measurement id, replay `references`/`confirm`); **probe red** (P2). | `tests/unit/Modules/GoogleDataManagerAbilitiesTest.php` |
| T105 | Low | addressed | Residue bundle: (a) `describe()` strip-tags pinned with a tag-free description (P8 green) — re-point to the containers field, pin `columns`/`rows_locked`; (b) `IMPORT_MAX_DEPTH` (P9 green) — 17-level payload; (c) `ConfigurationChecks` MonsterInsights branch never raised (P12 green) — both conflicts → two problems in order; (d) `StatusAbilities` schema-class-missing title fallback + junk `status_info` types; (e) `SettingsStore` scalar stored row; (f) `DestinationProbe` `?? new DestinationHealth()` effect unobserved + transport-failure/refused-exchange/failing-probe legs REST-only → small `DestinationProbeTest` or `[-]`; (g) harness: `AbilitiesTestCase::execute()` skips core's output validation (a schema drift is a live `ability_invalid_output` with a green suite; P11 red shows today's shapes are hand-pinned) — a small walker; `wp_register_ability` fake overwrites duplicates silently; `ContractTest:222` `method_exists` → `assertIsCallable`; (h) TS-2 pre-existing: `NoticesTest` stubs the escapers as identity — one hostile-name case with `stubEscapeFunctions()`; (i) rename `test_the_values_are_what_the_frontend_loads_not_the_raw_row`; (j) doc: `Registrar.php:44-46` credits `ContractTest` for the execute-time refusal pins that live in `RegistrarTest` + the provider tests; (k) optional: a secret-bearing entry through the log ability's own fixture (the allow-list is guarded one hop upstream in `SendLogTest:174`). **Closed 2026-09-22** — (a) placement description with `<code>` + the containers table shape, probe red P8; (b) a literal 17-level payload — the first draft sized it from the constant and stayed green under a raised cap (TS-21 inside a fixture), red after the literal, P9; (c) both conflicts in order + whole-list ordering, probe red P12; (d) junk-type and missing-class rows; (e) scalar row; (f) new `DestinationProbeTest` (8) incl. the built-in health fallback, probe red P13; (g) output-schema walker in `AbilitiesTestCase::execute()` — every ability result validated the way core does, the ability files' assertion counts roughly doubled —, duplicate-name fake fails, `assertIsCallable`; (h) `stubEscapeFunctions()` case both directions; (i) renamed; (j) `Registrar` docblock corrected; (k) smuggled `access_token`/`request_body` absent from the answer. | `tests/unit/Abilities/*`, `tests/unit/Admin/{ConfigurationChecks,Notices}Test.php`, `tests/unit/Modules/GoogleDataManager*Test.php` |
