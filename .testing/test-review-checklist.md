...
|---|---|---|---|---|---|---|---|
| **Plugin Bootstrap** (main file, `uninstall.php`, `Plugin.php`, `Autoloader.php`, **`src/RestCors.php`**) | `RestCorsTest` (5, incl. wrapper + non-request guard + priority-11 + namespace consistency); `Admin/AdminCapabilityFilterTest` (the `Plugin::boot()` gate, T30; + KeyNotice attachment, T74); **`PluginRestWiringTest`** (the `rest_api_init` closure — settings + google routes + RestCors, T66, probe-red); rest BE-3 N/A except `uninstall.php` (H-4) | [x] 2026-09-03 | [x] 2026-09-03 | [x] 2026-08-05 | [x] 2026-08-05 | [x] 2026-09-03 (T66 closed) | [x] 2026-09-03 |
...
| **Admin — Settings UI** (`src/Admin/SettingsPage.php` — boot-fallback notice `232fea6`, `Admin.php`, `RestController.php`, `PluginRow.php`, **`Docs.php` (2026-08-11)**) | `Admin/RestControllerTest`, `SettingsPageTest`, `PluginRowTest`, `AdminCapabilityFilterTest`, **`DocsTest`** (209 lines: allow-list `->with`, #152/#158 both-directions) + fixtures `UndocumentedThirdParty{Module,Schema}` | [x] 2026-09-02 | [x] 2026-09-02 | [x] 2026-09-02 (T60 closed — byte-exact containment, probe-red) | [x] 2026-09-02 | [x] 2026-09-02 | [x] 2026-09-02 |
...
| **Admin JS** (`js/admin/` — **13 files**: `App.js`, `utils.js`, `index.js` + **8 in `components/`** incl. `MobileNav.js` + `DocLink.js` + **2 in `components/panels/`** (ServiceAccountsPanel + registry, 2026-09-03; tracked in the GoogleAuth row above)) | **10** test files (+`service-accounts-panel`); `index.js` `[-]` N/A (3-line mount); `panels/index.js` covered via `module-panel.test.js` custom-panels (prototype-key fallback probe-red); `DocLink.js` `[-]` N/A documented-indirect — asserted by effect in `module-panel.test.js` incl. its one security prop (`rel="noopener noreferrer"`) | [x] 2026-09-03 | [x] 2026-09-03 | [x] 2026-09-03 | [x] 2026-09-03 | [-] | [x] 2026-09-03 (mockClear-not-mockReset adopted — see TC-15) |
...
| **Security-input coverage** (every `.security` PA-3/RI-2 sink has a hostile-input test — TS-1/TC-5) | 2026-09-03 (Run 10 + closes) | ✅ **Clean.** The google-auth range adds no dataLayer/HTML sink (REST JSON + an escaped admin notice, both hostile-pinned by the module's own suites). The new security surface is *egress*, not output: the SSRF allow-list now carries a 16-shape deny provider driven through both the pure check and the pre-socket seam (suffix-label, port, case, userinfo, path, subdomain, sibling-host…), the upload path keeps its 7-row hostile table + stores-nothing pin, and custody is asserted with a real sealed key in the store (anchored, T71). |
