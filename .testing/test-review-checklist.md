...
|---|---|---|---|---|---|---|---|
| **Plugin Bootstrap** (main file, `uninstall.php`, `Plugin.php`, `Autoloader.php`, **`src/RestCors.php`**, **`src/RequestOrigin.php`** — the shared guest-route gate, extracted Run 12, documented-indirect via `PageDataLayerTest` + `GoogleDataManagerBackfillTest`) | `RestCorsTest` (5, incl. wrapper + non-request guard + priority-11 + namespace consistency); `Admin/AdminCapabilityFilterTest` (the `Plugin::boot()` gate, T30; + KeyNotice attachment, T74); **`PluginRestWiringTest`** (the `rest_api_init` closure — settings + google routes + RestCors, T66, probe-red; + the GDM destinations route incl. callback identity AND the service-account deletion veto attach+effect from a real `boot()`, extended by the module's own author, Run 11); rest BE-3 N/A except `uninstall.php` (H-4; GDM health row pinned containment + exact four-key inventory) | [x] 2026-09-03 | [x] 2026-09-03 | [x] 2026-08-05 | [x] 2026-08-05 | [x] 2026-09-03 (T66 closed) | [x] 2026-09-03 |
...
| **Frontend Core** (`src/Frontend/` — DataLayer, ScriptTag (**`json_literal()` public static, 10 callers**), ContainerCode, ConsentDefaults, VisitorIp, **`DefaultLanguage` (2026-09-01, #145)**; Frontend = N/A) | `Frontend/DataLayerTest`, `ScriptTagTest`, `ContainerCodeTest`, `ConsentDefaultsTest`, `VisitorIpTest`, **`DefaultLanguageTest`** (exemplary TS-16 RunInSeparateProcess artifact) | [x] 2026-09-02 | [x] 2026-09-02 | [x] 2026-09-02 (T61/T63 closed — WPML null + empty-default + half-Polylang legs) | [x] 2026-09-02 (#225 noscript esc_attr untestable-red by construction — T65 note; wrong-fix direction pinned by BE-1 raw-`&` tests) | [x] 2026-09-02 | [x] 2026-09-02 |
| **Module Framework** (`src/Module/` — incl. **`DocumentedSchemaInterface`**, BE-3 N/A interface exercised via the doc sweep) | `Module/RegistryTest` (5, 6/6 public methods), `Modules/ModuleConsistencyTest` (+ **whole-schema doc sweep**; registry-driven, so EDD entered every sweep automatically with only the deliberate id-list acknowledgment changing — the design works), `ModuleHooksTest` | [x] | [x] 2026-09-02 | [x] 2026-09-02 | [-] | [-] | [x] 2026-09-02 |
| **PageVariables Module** (`src/Modules/PageVariables/` — post-meta allow-list + serialized skip + underscore floor, R27) | `Modules/PageVariablesModuleTest`, **`PageVariablesAdminSchemaTest`** | [x] | [x] 2026-09-02 | [x] 2026-09-02 (T63 primary-category omission pinned; T64 agreement oracle added) | [x] 2026-09-02 (#191 RI-26 floor + #198 guard-order + #194/#195/#197 all pinned discriminating) | [x] 2026-09-02 (T53 closed — #203 non-scalar case, probe-red) | [x] 2026-09-02 |
...
| **Google API client** (`src/Google/` — **6 files** (ConsentRegions added Run 12): KeyVault 555 (clock seam added Run 10), ServiceAccountKey 179, TokenService 258, Transport 56 (interface, BE-3 N/A; its phase-2 methods ARE allow-list-tested), WpTransport 168 (SSRF egress guard; port refusal added Run 10). Landed 2026-09-03 `ea7ece7`, **row added Run 10**) | `Google/KeyVaultTest` (30), `TokenServiceTest` (26, incl. a RunInSeparateProcess sign-failure case), `ServiceAccountKeyTest`, `WpTransportTest` (16-row deny provider ×2 tests + seam), helpers `FakeTransport` (enforces the real allow-list, TS-13), `KeyFileFixture` (real RSA pair per process), `OptionStoreTrait` (core-faithful add/update semantics incl. unchanged-value false), `ConsentRegionsTest` (exact set + count, EU-27 written out independently of the source constant) | [x] 2026-09-03 | [x] 2026-09-03 | [x] 2026-09-03 (T70/T71 closed; openssl no-extension legs NOTE-documented untestable in-process) | [x] 2026-09-03 (deny-before-socket ×16 shapes ×3 methods; look-alike suffix + port + case rows) | [x] 2026-09-03 (#226/#227/#228 all revert-traced or probe-red; #231 wontfix no guard owed) | [x] 2026-09-03 |
...
| **Admin — Settings UI** (`src/Admin/SettingsPage.php` — boot-fallback notice `232fea6`, `Admin.php`, `RestController.php`, `PluginRow.php`, **`Docs.php` (2026-08-11)**) | `Admin/RestControllerTest`, `SettingsPageTest` (+ columnChoices merge both directions, Run 11), `PluginRowTest`, `AdminCapabilityFilterTest` (+ KeyNotice AND HealthNotice boot attachments — T74/T75, both probe-red), **`DocsTest`** (209 lines: allow-list `->with`, #152/#158 both-directions) + fixtures `UndocumentedThirdParty{Module,Schema}` | [x] 2026-09-02 | [x] 2026-09-02 | [x] 2026-09-02 (T60 closed — byte-exact containment, probe-red) | [x] 2026-09-02 | [x] 2026-09-03 (T75 closed — the `Admin::boot()` HealthNotice wiring was deletable green, the T74 lesson recurring for the sibling notice in the same range) | [x] 2026-09-02 |
...
| **Frontend JS** (`js/frontend/` — **21** bundles (+`gtm4wp-edd.js` 714 lines, 2026-09-01) + **3** libs: `lib/native-video-params`, `lib/blocks-cart-diff`, `lib/gtm4wp-cookies`) | **27** `js/frontend/test/*.test.js` (+`edd-tracker.test.js`, 15 tests); every bundle covered. `build/gtm4wp-edd.js` has **0** `"use strict"` — no `toThrow` assertions used (R26/L18 compliant) | [x] 2026-09-02 | [x] 2026-09-02 (T62 closed — API grant/deny) | [x] 2026-09-02 (T55/T62 closed — absence-of-push shapes) | [x] 2026-09-02 (#463 discriminating cart/checkout split — payment store mocked PRESENT; TC-11 applies) | [x] 2026-09-02 (T54 closed — #222 pinned, probe-red) | [x] 2026-09-02 (T56 closed — T33 detach harness adopted) |
| **Admin JS** (`js/admin/` — **14 files**: `App.js`, `utils.js`, `index.js` + **8 in `components/`** incl. `MobileNav.js` + `DocLink.js` + **3 in `components/panels/`** (ServiceAccountsPanel + **DestinationsPanel (2026-09-03, `f0b3667`)** + registry)) | **11** test files (+`destinations-panel` 27 tests); `index.js` `[-]` N/A (3-line mount); `panels/index.js` covered via `module-panel.test.js` custom-panels incl. the two-entry discrimination + DOM-order pin; `DocLink.js` `[-]` N/A documented-indirect | [x] 2026-09-03 | [x] 2026-09-03 | [x] 2026-09-03 (T80 closed — error-leg symmetry, malformed resolve **surfaced+fixed a latent panel crash**, threshold boundary, type default) | [x] 2026-09-03 | [x] 2026-09-03 (#234 fingerprint trio + must-not-break verified; T76 busy-guard probe-red — prop layer; handler re-check stays the TC-15 r7 blind spot) | [x] 2026-09-03 (…Once mocks — TC-15 r6 corollary) |
...
|---|---|---|
| **Test-order independence** (`vendor/bin/phpunit --order-by=random` — TS-8/TS-16) | 2026-09-03 (Run 11 + closes) | ✅ **Clean — declaration + 3 random seeds green at 2397/6552 after the closes** (2394/6539 + 3 seeds at report stage), identical counts. The new GDM suites stub `wp_parse_url` and their own store in-file with TS-16 comments; no new hazard in range. |
...
| **Untested public methods** (in classes that *do* have a test file — TS-10) | 2026-09-03 (Run 11 + closes) | ✅ **Clean after the same-session closes** — the two uncovered methods the coverage report corroborated are pinned: `AdminSchema::panel()` (T78, the PHP↔JS registry-key drift guard) and the HealthNotice boot attachment (T75, probe-red). Null-clock fallback legs recorded `[-]` N/A. |
