...
* Fixed: three option descriptions on the settings screen (Page variables -> Post data) described the output incorrectly. "Category list of current post/archive" said it sends the category *names*, while `pageCategory` has always contained the category **slugs** (`news-and-events`, not `News and Events`) - so a Google Tag Manager trigger written against the description could never match. "Tags of current post" had the same two gaps: it named neither the variable it writes (`pageAttributes`, not `pageTags`) nor the fact that tags are also listed by slug. "Post date" said it adds 4 data layer variables, while it has added 9 since 1.15: `pagePostDate`, `pagePostDateYear`, `pagePostDateMonth`, `pagePostDateDay`, `pagePostDateDayName`, `pagePostDateHour`, `pagePostDateMinute`, `pagePostDateIso` and `pagePostDateUnix`. All three descriptions now match what is sent; no data layer value changed.
