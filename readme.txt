...
* Fixed: with a custom data layer variable name, the Google Consent Mode default block pushed its defaults to dataLayer instead of the configured variable, so the consent defaults did not apply. The block now uses the configured name and stays out of WP Rocket's combined JavaScript.
