...
* Fixed: repeated PHP warnings (`Attempt to read property "post_author" on null`) on sites where a theme or plugin leaves the global post object unset on a singular page. The post-sourced page variables (post author, term list, page hierarchy) are now simply omitted on such a request, as reported on the wordpress.org support forum against 1.22.3.
