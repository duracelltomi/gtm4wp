...
* Changed: on the order received page, the customer details in the `orderData` variable and the Enhanced Conversions `user_data` block on the `purchase` event are now left out whenever WooCommerce itself would not show the order to that visitor. Since WooCommerce 8.4 that page asks a registered customer to log in before it shows their order, and asks a guest to confirm the billing email address once the order is older than the verification grace period (ten minutes by default), while GTM4WP still built the data layer from the order key alone - so a forwarded or otherwise leaked confirmation link handed the buyer's name, address, email and phone number to whoever opened it. The `purchase` event itself, the order totals and the items are unchanged and still fire on the order key as before, so nothing is lost for the buyer arriving straight from checkout, who is either logged in or well inside the grace period. WooCommerce's own `woocommerce_order_received_verify_known_shoppers` and `woocommerce_order_email_verification_grace_period` filters are both honoured, so a store that has relaxed either check keeps the matching behavior here. A custom thank-you page is unaffected: it resolves the order from the buyer's own session rather than from a link, so its customer data is always complete.
