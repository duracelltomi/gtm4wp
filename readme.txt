...
* Fixed: the hashed phone number in orderData.customer.billing.phone_hash is now converted to E.164 (+36201234567) before hashing, which is the format Google matches on. Previously the number was only lowercased and stripped of spaces, so anything a customer typed the way they dial it - 06 20 123 4567, (555) 123-4567 - produced a hash Google could never match, and the enhanced conversion was simply left unattributed with no error shown anywhere. The billing country of the order now supplies the calling code and the national trunk prefix rule; when the number cannot be placed in E.164 with confidence - no country on the order, or too few digits to be a real number - the key is sent empty instead of carrying an unmatchable hash. **Check your GTM setup if it reads phone_hash:** the value changes for every customer who did not type their number in international format.
