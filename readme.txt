...
* Changed: GTM4WP no longer adjusts WordPress' inline-style sanitizing rules for the whole page load. The container's hidden `<noscript>` iframe needs the `display` and `visibility` CSS properties to survive WordPress' HTML sanitizer, and that permission used to be granted for the entire request, so it applied to everything else WordPress sanitized on the same request as well. It is now granted only while the plugin sanitizes its own container markup and withdrawn immediately afterwards. The container code that reaches the page is unchanged; post content and other markup WordPress sanitizes are held to WordPress' own inline-style rules again, exactly as they would be without the plugin installed.
