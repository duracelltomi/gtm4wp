...
{
	global $gtm4wp_options, $gtm4wp_container_code_written, $gtm4wp_nonce;
...
<!-- Google Tag Manager (noscript) -->';
	...
	if ((GTM4WP_PLACEMENT_OFF === $gtm4wp_options[GTM4WP_OPTION_GTM_PLACEMENT]) && (!$no_console_log)) {
		...
		$_gtm_tag .= '
<script' . ($has_html5_support ? '' : ' type="text/javascript"') . ($add_cookiebot_ignore ? ' data-cookieconsent="ignore"' : '') . ($gtm4wp_nonce ? ' nonce="' . esc_attr($gtm4wp_nonce) . '"' : '') . '>
...
{
	global $gtm4wp_options, $gtm4wp_datalayer_name, $gtm4wp_nonce;
...
<script data-cfasync="false" data-pagespeed-no-defer' . ($has_html5_support ? ' type="text/javascript"' : '') . ($add_cookiebot_ignore ? ' data-cookieconsent="ignore"' : '') . ($gtm4wp_nonce ? ' nonce="' . esc_attr($gtm4wp_nonce) . '"' : '') . '>
...
{
	global $gtm4wp_datalayer_name, $gtm4wp_datalayer_data, $gtm4wp_options, $gtm4wp_nonce;
...
<script' . ($has_html5_support ? '' : ' type="text/javascript"') . ($add_cookiebot_ignore ? ' data-cookieconsent="ignore"' : '') . ($gtm4wp_nonce ? ' nonce="' . esc_attr($gtm4wp_nonce) . '"' : '') . '>
	...
	if (('' !== $gtm4wp_options[GTM4WP_OPTION_GTM_CODE]) && $output_container_code) {
		...
		foreach ($_gtm_codes as $one_gtm_id) {
			...
			echo '
<script data-cfasync="false"' . ($add_cookiebot_ignore ? ' data-cookieconsent="ignore"' : '') . ($gtm4wp_nonce ? ' nonce="' . esc_attr($gtm4wp_nonce) . '"' : '') . '>
