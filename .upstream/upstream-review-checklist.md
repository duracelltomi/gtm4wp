...
directions, and only one of them fails **open**. If the helper is renamed, moved out of
`Internal` or dropped, the `class_exists`/`method_exists` guard resolves to "not
available", the guest half stops applying, and the data layer reverts to the older,
weaker condition — with no error and a green suite, because the tests drive our own
stub, which cannot notice upstream moving. The known-shopper half fails the other way:
a renamed filter means we withhold on a site that turned the gate off, which is merely
missing data. **Re-verify the helper first**, and treat a green suite as no evidence
either way.
