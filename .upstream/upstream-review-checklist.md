...
| U134 | **The Google Analytics `refund` recommended event, as the Data Manager API expresses it.** Field-by-field: `eventName` = `refund`, `eventTimestamp` (RFC 3339), `transactionId` (the join key), `currency` + `conversionValue` as a required pair, `cartData.items[]` with `itemId` / `unitPrice` / `quantity`, `clientId`, `eventSource` = `WEB`, and the returned **shipping and tax as `additionalEventParameters` entries** (`{parameterName, value}` pairs under the Google Analytics parameter names, values as strings) - neither has a field of its own on the event, and a refund that omitted them let Analytics' shipping and tax metrics be incremented by the purchase and never decremented. The plugin sends **two shapes** from this: a whole-order refund carries the transaction id alone, a partial one carries the value pair and the items. ⚠ **Known tension to re-read, not a settled fact:** the Data Manager recommended-events reference marks `currency` and `conversionValue` as required for `refund`, while the transaction-id-only full refund comes from the gtag `refund` contract; the two-shape mapping is a deliberate maintainer decision and this row is where it gets re-checked. Claim to re-verify: the field names above still spell the same thing, and the reference has not started requiring the value pair for the full-refund case in a way that would make our full-refund event a no-op | `src/Modules/GoogleDataManager/RefundEvent.php` | https://developers.google.com/data-manager/api/reference/analytics/recommended-events (refund row) + https://developers.google.com/analytics/devguides/collection/ga4/reference/events#refund | the API's own release notes (U119) | **silent** — a renamed field is dropped by Google with a 200 answer, and the contract tests only pin OUR side | every-run | [x] 2026-09-08 re-read after the phase 4 acceptance run found the shipping gap: `shipping` and `tax` are listed as event parameters for both `purchase` and `refund` on the recommended-events reference, and the send-events devguide gives the `additionalEventParameters` `{parameterName, value}` shape (https://developers.google.com/data-manager/api/devguides/events/send-events). Found in the field, not by a sweep: the row pinned the fields we send, never the fields we do not - a partial refund of shipping reported the money in `conversionValue` and said nothing about shipping. Prior: [x] 2026-09-06 field list and the required-pair marking read from the recommended-events reference |
