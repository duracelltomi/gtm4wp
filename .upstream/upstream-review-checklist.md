...
| U97 | **The cart-fragments delivery channel** (cache-safe data layer only). Three separate claims, all silent if wrong: (a) the `woocommerce_add_to_cart_fragments` filter still exists and its selector⇒HTML contract is unchanged; (b) WC still re-applies fragments from its own `sessionStorage` cache on a normal page load, which is what delivers the block when no cart mutation happened; (c) **the `wc-cart-fragments` script is actually enqueued on the page** — nothing on our side enqueues it, so on a store that never loads it (no mini-cart) the customer/cart block never arrives at all and no error is raised. Claim (c) is the one that is *not* verified and must not be assumed from "nobody complained" (UD-11) | `src/Modules/WooCommerce/PageDataLayer.php` (`add_visitor_cart_fragment`, `output_visitor_cart_placeholder`), `src/Modules/WooCommerce/WooCommerceModule.php`, `js/frontend/gtm4wp-visitor-data.js` | WC `includes/wc-cart-functions.php` / `class-wc-frontend-scripts.php` + `assets/js/frontend/cart-fragments.js` | U77, U70 | silent-missing | on-WC-release | [ ] |
...
| U71 | PHP↔JS name contracts: `gtm4wp_item_list_attr`, `gtm4wp_orderid_tracked`, `gtm4wp_woo_event`, `gtm4wp_readded_to_cart`, `div.gtm4wp-wc-visitor-data`, the cart-fragment payload's `customer`/`cart` envelope keys, and the three data layer event names in `config.events` (`VisitorDataModule::EVENT_*` ↔ the `resolveEventName()` fallbacks) (UC-4, internal) | `src/Modules/WooCommerce/{Helpers,ProductData,PageDataLayer}.php`, `src/Modules/VisitorData/VisitorDataModule.php` ↔ `js/frontend/*.js` | internal grep | — | silent-missing | every-run | [ ] |
