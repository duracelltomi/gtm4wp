...
| U115 | **EDD frontend contract the EDD module binds to** — markup-injection hooks `edd_download_after`, `edd_purchase_link_end( $download_id )`, `edd_checkout_cart_item_title_after( $item, $key )`; selectors `.edd-add-to-cart`, `form.edd_download_purchase_form`, `.edd_price_options input` (input value = price id), `a.edd_cart_remove_item_btn` inside `tr` cart rows, `input[name=payment-mode]`, `#edd-purchase-button`, grid wrappers `.edd_download` / `.edd-blocks__download`; jQuery body event `edd_gateway_loaded` (first firing = checkout init, not a buyer pick) | `src/Modules/EasyDigitalDownloads/ListTracking.php`, `src/Modules/EasyDigitalDownloads/PageDataLayer.php`, `js/frontend/gtm4wp-edd.js` | EDD releases (`awesomemotive/easy-digital-downloads`); claims researched at 3.6.9 (`docs/dev/edd-integration-plan.md`), re-read 2026-08-14 from a local 3.3.5.2 checkout | — | silent-missing | quarterly | [x] 2026-08-14 against EDD 3.3.5.2 source: classic templates and the checkout block both fire the cart-row hook and keep the `payment-mode`/`edd-gateway` inputs and the `edd_cart_remove_item_btn` class; the blocks (div cart rows, `edd/cart` page detection, `edd_blocks_downloads_*` item hooks) are scoped out of v1 per the plan doc's backlog. The module's PHP-side EDD API couplings (receipt payment-key chain, cart/order accessors, `edd_get_payment_statuses`) still need their own registry rows |
