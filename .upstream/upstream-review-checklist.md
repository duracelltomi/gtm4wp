...
| U139 | **Action Scheduler's public function API**, used as the send queue wherever WooCommerce provides it: `as_schedule_single_action( $timestamp, $hook, $args, $group )` (returns the action id, 0 on refusal), `as_next_scheduled_action()`, `as_get_scheduled_actions( $args, $return_format )` and `as_unschedule_all_actions( $hook, $args, $group )`. Every one is called behind `function_exists`, and the WP-Cron path is a full alternative rather than a degraded one — Easy Digital Downloads bundles no Action Scheduler, so that path is the only one on an EDD-only store. Two secondary claims: the `'status' => 'pending'` literal the queue-depth query passes (the documented function, deliberately not its `ActionScheduler_Store` constant, which is an internal), and that scheduled arguments still reach the callback as a positional list | `src/Modules/GoogleDataManager/SendQueue.php` | https://actionscheduler.org/api/ + the WooCommerce release the bundled copy ships in | WooCommerce betas/RCs (U29) | loud on our side (a removed function falls back to WP-Cron), **silent** for the queue-depth row (an unrecognised status filter simply counts nothing) | quarterly | [x] 2026-09-06 signatures read from the Action Scheduler API reference |
