...
| WooCommerce | **11.1.0** [x] 2026-09-07 (S2) | [ ] none open | — | `WC requires at least: 5.0` / `WC tested up to: 11.1.0` | Claim bumped 11.0.0 → **11.1.0** on 2026-09-07 after a full re-probe of all 22 `on-WC-release` rows against the 11.1.0 source, the 11.1.0 changelog read for deprecations and behaviour changes (1 relevant: #66748, assessed benign — see U121), and a maintainer runtime run of 2.0.1 on a live 11.1 shop. 1 new finding: D13 (dead hook registration, pre-existing) |
...
| U16 | `WC tested up to: 11.1.0` claim | plugin header | wp.org plugin API | U77 | deprecation | every-run | [~] 2026-09-07 (S2) bumped 11.0.0 → **11.1.0**. Source-verified across all 22 `on-WC-release` rows against the 11.1.0 source; deprecation audit of the 11.1.0 changelog clean for our surface (the three deprecations it announces — `woocommerce_coupon_object_updated_props`, `WC_Coupon_Data_Store_CPT::$updated_props`, retired WC-Admin feature flags — touch nothing we call); rule 3 satisfied and then strengthened: the maintainer ran 2.0.1 on a live 11.1 shop, and on 2026-09-07 exercised the specific paths rather than only the shop — block-cart quantity batching and mixed mutations (U121, the one 11.1 behaviour change, now measured rather than inferred) and a real purchase on the order-received page (U113, the largest coupling surface). The interactive Add to Cart + Options paths were **not** exercised: upstream does not use that block in the default 11.1 Single Product template and still ships it as Beta, so reaching it needs a deliberate template edit. Still `[~]`, not `[x]`: **no automated runtime coverage against any WC 11.x** (U92) |
...
| U24 | ~40 classic WC DOM selectors | `js/frontend/gtm4wp-woocommerce.js` | WC templates (unversioned) | U77 | silent-missing | on-WC-release | [x] 2026-09-07 (S2) **first complete probe** (the 2026-08-06 run reached 14 tokens and left this `[~]`). Counting rule: every `.`-prefixed class token inside a single-quoted string in the tracker → **31**, each resolved against `templates/`, `includes/`, `src/`, `assets/client/`, `assets/js/` at 11.1.0. 24 present as literals. 2 composed at runtime and verified at their composition site, not by grep: `product_type_variable` / `product_type_grouped` (`'product_type_' . $product->get_type()`, `wc-template-functions.php:1479` and `ProductButton.php:122`) and `woocommerce-grouped-product-list-item__label` (`grouped.php:117`). 5 are not WooCommerce's and are correctly absent: our own `gtm4wp_productdata` / `gtm4wp_single_productdata`, third-party `quick-view-button` (U34) and `product_type_bundle_input_required`. **1 is dead**: `widget-product-item` is in no WooCommerce version probed (9.0.0, 11.0.0, 11.1.0) and nothing in this plugin emits it; benign, since it is an OR arm beside `.gtm4wp_productdata`, which carries the match |
...
| U27 | WC template filename literal `content-widget-product.php` | `src/Modules/WooCommerce/ListTracking.php` | WC templates | U77 | silent-missing | on-WC-release | [x] 2026-09-07 (S2) re-verified at **11.1.0** |
...
| U126 | **WooCommerce terminal order statuses `failed` / `cancelled` / `refunded`**, mirrored in `ProductData::PENDING_PURCHASE_TERMINAL_STATUSES`: an order remembered for the pending-purchase re-check (2.0.3, the Stripe-with-webhooks case) is dropped in one of these and re-checked in any other status. Four claims, all read from the 11.1.1 source: (1) `wc_get_order_statuses()` still lists exactly pending / processing / on-hold / completed / cancelled / refunded / failed, so these three are the only core statuses with no path to a paid one — a status added upstream would be re-checked harmlessly until the window closes (the benign direction), a renamed one would stop being recognised as terminal (same effect); (2) `WC_Shortcode_Checkout::order_received()` unsets `WC()->session->order_awaiting_payment` **unconditionally, right after its order-key check and before the template renders**, so on the classic order received page WooCommerce's own marker is gone before the customer's next page view and `PageDataLayer::pending_session_order_id()`'s fallback read of it can never carry a pending order across; `WC_Order::payment_complete()` sets it `false`, `WC_Cart_Session::destroy_cart_session()` (on `woocommerce_cart_emptied`) nulls it, the block Order Confirmation (`AbstractOrderConfirmationBlock`) reads only `store_api_draft_order` and clears nothing, and the Store API `Checkout` route never writes `order_awaiting_payment` at all; (3) `wc_cancel_unpaid_orders()` moves a still-pending order to `cancelled` after `woocommerce_hold_stock_minutes` (default 60, filter `woocommerce_cancel_unpaid_orders_interval_minutes`), the natural end of a re-check for an abandoned order; (4) `WC_Abstract_Order::get_status()` returns the stored status without the `wc-` prefix, which is the form compared here | `src/Modules/WooCommerce/ProductData.php` (`PENDING_PURCHASE_TERMINAL_STATUSES`, `PENDING_PURCHASE_RECHECK_WINDOW_MINUTES`, `may_become_trackable()`), `PageDataLayer::pending_session_order_id()` | `includes/wc-order-functions.php`, `includes/shortcodes/class-wc-shortcode-checkout.php`, `includes/class-wc-order.php`, `includes/abstracts/abstract-wc-order.php`, `includes/class-wc-cart-session.php`, `src/Blocks/BlockTypes/OrderConfirmation/AbstractOrderConfirmationBlock.php`, `src/StoreApi/Routes/V1/Checkout.php`; pinned from our side by `ProductDataTest::test_terminal_statuses_and_the_recheck_window_are_pinned` (exact set + window) | U77 | silent-missing (bounded: at worst a pending order keeps waiting until the window or the "Maximum order age" closes it) | on-WC-release | [x] 2026-09-23 read at **11.1.1**, registered with the fix |
...
| U35 | WP Rocket `rocket_excluded_inline_js_content` — the list is matched against every inline script; claims to verify: substring vs regex matching, WP Rocket's own default entries (`gtag`? `dataLayer`?), and where combined inline content lands relative to the original position. Since 2026-09-23 the plugin adds `ConsentDefaults::CONSENT_DEFAULT_COMMAND` so the consent block stays covered under a custom data layer name (`.security` #325, RI-34) | `src/Frontend/ContainerCode.php`, `src/Frontend/ConsentDefaults.php` | WP Rocket | — | silent-wrong (a block dropped from the list is combined and moved with no error; was `loud` until 2026-09-23) | on-demand | [ ] |
...
| U168 | **Release zip top folder == the wp.org slug `duracelltomi-google-tag-manager`** — core's upload flow detects an installed plugin by folder, so a zip unpacking to the long `-for-wordpress` git name installs as a second copy beside a wp.org install (silent; GitHub zips before the 2.0.3 re-upload did this). NOT the update check: `update-check/1.1` falls back to the `Plugin Name` header, so a long-folder install IS offered wp.org updates (measured 2026-09-24: no headers → not recognized, Name alone → matched; Text Domain or Plugin URI alone → not). What the update then does to a long-folder install (folder move, reactivation) is unmeasured. Main file keeps the long name, as in SVN | `tools/build-release.js` `WPORG_SLUG` (folder + zip name); plugin header `Plugin Name` (the update-check match key for a non-slug folder) | `api.wordpress.org/plugins/update-check/1.1/` + wp.org download zip top folder | — | silent-missing | pre-release | [x] 2026-09-24 registered; pinned to the `Text Domain` header (U97) by `tests/unit/ReleaseZipSlugTest.php`; re-uploaded 2.0.3 zip verified to unpack to `duracelltomi-google-tag-manager/`. Renaming `Plugin Name` would drop update offers for every long-folder install |
...
| **Behavioral contracts (A: U3–U14, B: U15–U30) against vendor source** | 2026-09-07 (S2) | ✅ **Section B only** (WC-scoped): all 22 `on-WC-release` rows re-derived against WC **11.1.0**, unpacked from the wp.org package. 1 finding (D13, pre-existing dead registration, not 11.1 drift), 1 stale row text corrected (U30), and U24 closed at last — `[~]` → `[x]` on a 31-token measurement, the first complete probe of that row. The one 11.1.0 behaviour change reaching a registered coupling (#66748, event batching) was assessed against both consumers and is benign; recorded in U121 with the reason, so the next sweep does not re-derive it. **Section A (WP) not re-run** — WP has not moved since 2026-08-06. **Lesson, second time: the wp.org package is the probe.** One download turned 22 rows into local greps, and the two rows that needed a fetch beyond it (U26, U121) needed it only because their source is TypeScript that ships built. (2026-08-06 (S1a): ⚠️ First real run. **24 rows `[ ]` → `[x]`** against WP 7.1-RC1 + WC 11.0.0 extracted locally. 3 findings, all "dead compensation that still reads as load-bearing": D10 (`safe_style_css` gained `display` in 7.0, `visibility` in 7.1 — our filter still needed at the floor), D11 (`woocommerce_related_products_args` never existed in our range), D12 (`wc-` vs `wp-` cross-sell class). U5 inconclusive, U9 is not a core coupling. **Lesson: these were never fetch-bound — download the vendor source and most of section A/B is a local grep.** |
