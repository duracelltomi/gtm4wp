...
| U110 | **National trunk prefixes that are not a single `0`** — 5 entries (`HU`→`06`, `RU`/`KZ`/`BY`/`LT`→`8`) plus the `0` default. A mirror of national numbering-plan facts, needed to turn a locally-typed phone into E.164. Claim to re-verify: those five are still right, **and no other territory in WooCommerce's country list needs an entry**. North America is deliberately absent (trunk prefix and calling code are both `1`, so the generic branches cover it) | `src/Modules/WooCommerce/Helpers.php:582` `NATIONAL_TRUNK_PREFIXES` | ITU / national numbering plans; no single canonical page, which is itself the risk | — | silent-wrong | yearly | [x] 2026-08-10 written from the numbering plans at the time of the fix; pinned from our side by the per-country cases in `HelpersTest` (HU multi-digit, RU non-zero, DE single zero, US none) |
| U111 | **`WC_Countries::get_country_calling_code()` return shape** — prefixed with `+`, an **array** for territories carrying more than one code, and `''` for a country it does not know. We delegate the ~200-entry calling-code table to WooCommerce rather than mirroring it, so this method's *shape* is the coupling. Available since WC 3.6, below our 5.0 floor, and reached through `property_exists`/`method_exists` guards so a removal degrades to "no phone hash" rather than a fatal | `src/Modules/WooCommerce/Helpers.php:609` `country_calling_code()` | https://woocommerce.github.io/code-reference/classes/WC-Countries.html | WC pre-releases (U84/U85) | silent-missing | quarterly | [x] 2026-08-10 signature `get_country_calling_code( string $cc ): string\|array`, "stripped of - and spaces and always be prefixed with +", `@since 3.6.0`. Test double in `tests/unit/Modules/wc-stubs.php` is held to the same shape incl. the unknown-country `''` (UC-3) |
...
- **The anchor half is deliberately `[~]`, not `[ ]` or `[x]`.** The paths are verified;
  the anchors are known-partial and the row says which pages are outstanding. The work
  left is mechanical (add `<a name="<option key>"></a>` to the heading that documents
  each option, as the WooCommerce settings reference does) and the helper used for it
  lives in the session scratchpad, not in the repo — it is a one-off, not a tool.
  Finishing it changes no plugin code: every link already opens the right page.
