...
| U168 | **Release zip top folder == the wp.org slug `duracelltomi-google-tag-manager`** — core keys an install's update check on `<folder>/<main file>`, so a GitHub zip unpacking to the long `-for-wordpress` git name installs as a plugin wp.org never updates (silent; GitHub zips up to 2.0.3 did this). Main file keeps the long name, as in SVN | `tools/build-release.js` `WPORG_SLUG` (folder + zip name) | `api.wordpress.org/plugins/info/1.2/` slug + wp.org download zip top folder | — | silent-missing | pre-release | [x] 2026-09-24 registered; pinned to the `Text Domain` header (U97) by `tests/unit/ReleaseZipSlugTest.php`; first zip built with it verified to unpack to `duracelltomi-google-tag-manager/` |
