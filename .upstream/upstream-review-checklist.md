...
| U115 | **Consent Mode default `region` EEA preset — 32 codes (counting rule: `count(ConsentDefaults::REGIONS_EEA_UK_CH)`), EU-27 + IS/LI/NO + GB + CH — plus the `wait_for_update` / `ads_data_redaction` / `url_passthrough` parameter names.** The list is a UD-1 mirror with no single canonical Google page: Google documents that `region` takes ISO 3166-2 codes but publishes no official EEA list, so the mirrored claim is "the region set that independent Google-tag implementations converge on" — corroborated against four of them, byte-identical, verified 2026-08-11. **The sources are named only in the git-ignored local research notes (`.roadmap/`), deliberately: which implementations we read is not published.** Drift signal is a membership change in those sources, or the UK/CH political question being answered differently by Google (both are in every shipped list today despite not being EEA members). `wait_for_update: 500` is the value all four carry; ours is user-configurable with 500 recommended in the field description. Pinned from our side by the exact-set test in `tests/unit/Frontend/ConsentDefaultsTest.php` (a silent edit to the constant fails the pin) | `src/Frontend/ConsentDefaults.php` `REGIONS_EEA_UK_CH`, `payload()`, `script_block()` | https://developers.google.com/tag-platform/security/guides/consent (parameter names); list membership corroborated in the git-ignored `.roadmap/` notes | — | silent-wrong | quarterly | [x] 2026-08-15 mirrored from the four-source convergence recorded 2026-08-11; parameter names read from the consent docs. **[x] 2026-08-16 three further claims the emitted block now rests on, all read from the advanced-mode guide:** (1) **no ordering is imposed** between `gtag('set','ads_data_redaction'/'url_passthrough')` and the `consent default` command — the only placement rule given is that `url_passthrough` precede any `config` commands, which this plugin never emits; implementations in the wild are split on which side they emit, so a future "fix" to match one of them would be chasing a non-requirement; (2) the redaction is **scoped by Google itself** — "To further redact your ads data when `ad_storage` is `denied`, set `ads_data_redaction` to `true`" — which is why a static `true` is emitted once instead of being re-emitted per consent update; (3) region precedence is **most-specific-wins and replaces** ("the one with a more specific region will take effect"), a no-region command is the fallback "for all visitors not covered by another region-specific command", and `update` commands are **not** region-scoped. **Deliberately NOT relied on** (undocumented, re-check if it ever gets documented): what an unset/unmatched consent type does, and whether `wait_for_update` interacts with regions |
