...
| Google Data Manager API (U119) | **v1.8** (2026-07-30) [x] 2026-09-03 | — (Google publishes no pre-release channel; a version appears on the reference page when it ships) | — | U123–U125 — the `google-data-manager` module now couples to the `events:ingest` request shape, the GA `Destination` shape and the request-level limits | re-read the reference page's newest entry each sweep; a bump is the only warning before a request shape moves |
