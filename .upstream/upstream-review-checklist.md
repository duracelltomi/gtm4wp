...
| Google Data Manager API (U119) | **v1.8** (2026-07-30) [x] 2026-09-03 | — (Google publishes no pre-release channel; a version appears on the reference page when it ships) | — | nothing yet — phase 1 allow-lists the host only; the `/v1` request shapes couple in the `google-data-manager` module | re-read the reference page's newest entry each sweep; a bump is the only warning before a request shape moves |
